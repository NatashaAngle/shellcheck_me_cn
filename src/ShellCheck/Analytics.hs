...
mkChecker spec params checks =
  ...
  where
    all = checks 在 POSIX sh 中，未定义 ++ 。 optionals
...
runAndGetComments f s = do
        在 POSIX sh 中，未定义 let。 pr = pScript s
        root <- prRoot pr
        在 POSIX sh 中，未定义 let。 spec = defaultSpec pr
        在 POSIX sh 中，未定义 let。 params = makeParameters spec
        return $
...
replaceStart id params n r =
    在 POSIX sh 中，未定义 let。 tp = tokenPositions params
        (start, _) = tp Map.! id
        new_end = start {
...
getAlias arg =
    在 POSIX sh 中，未定义 let。 string = onlyLiteralString arg
    in when ('=' `elem` string) $
...
checkEchoWc _ (T_Pipeline id _ [a, b]) =
  ...
  where
    acmd = oversimplify a
    bcmd = oversimplify b
    countMsg = style id 2000 "请确认是否可改用 ${#variable}."
...
checkPipedAssignment _ (T_Pipeline _ _ (T_Redirecting _ _ (T_SimpleCommand id (_:_) []):_:_)) =
    warn id 2036 "如果您想赋值管道的输出，请使用 a=$(b | c)。"
    ...
    check op =
        when (op `elem` ["+", "-", "*", "/"]) $
            warn (getId firstWord) 2099 $
                "Use $((..)) for arithmetics, e.g. i=$((i " 在 POSIX sh 中，未定义 ++ 。 op 在 POSIX sh 中，未定义 ++ 。 " 2))"
...
checkPipePitfalls _ (T_Pipeline id _ commands) = do
    for ["find", "xargs"] $
        \(find:xargs:_) ->
          在 POSIX sh 中，未定义 let。 args = oversimplify xargs 在 POSIX sh 中，未定义 ++ 。 oversimplify find
          in
            unless (any ($ args) [
                hasShortParameter '0',
                hasParameter "null",
        ...
        \(ps:grep:_) ->
            ...
            in
                ...
                unless (any (`elem` ["p", "pid", "q", "quick-pid"]) psFlags) $
                    info (getId ps) 2009 "SC2009 考虑使用 pgrep 代替 grep ps 输出。"

    for ["grep", "wc"] $
        \(grep:wc:_) ->
            在 POSIX sh 中，未定义 let。 flagsGrep = maybe [] (map snd . getAllFlags) $ getCommand grep
    ...
    didLs <- fmap or . sequence $ [
        for' ["ls", "grep"] $
            \x -> warn x 2010 "不要使用 ls | grep。请使用 glob 或条件 for 循环来允许使用非字母数字文件名。",
        for' ["ls", "xargs"] $
    ...
    unless didLs $ void $
        for ["ls", "?"] $
            \(ls:_) -> unless (hasShortParameter 'N' (oversimplify ls)) $
                info (getId ls) 2012 "使用 find 来代替 ls 来更好地处理非字母数字文件名。"
  where
    for l f =
        在 POSIX sh 中，未定义 let。 indices = indexOfSublists l (map (headOrDefault "" . oversimplify) commands)
        in do
...
indexOfSublists sub = f 0
  where
    f _ [] = []
    f n a@(r:rest) =
        在 POSIX sh 中，未定义 let。 others = f (n+1) rest in
            if match sub a
              then n:others
              else others
...
checkShebangParameters _ (T_Script _ (T_Literal id sb) _) =
    [makeComment ErrorC id 2096 "在大多数操作系统中，shebang 只能指定单个参数。" | isMultiWord]
  where
...
checkShebang params (T_Script _ (T_Literal id sb) _) = execWriter $ do
    ...
    unless (null sb) $ do
        ...
        when ("/" `isSuffixOf` head (words sb)) $
            err id 2246 "当前 shebang 指定了一个目录。请确保解释器是一个文件。"
    ...
    forM_ multiple $ \arg -> sequence_ $ do
        ...
        return $
            warnWithFix (getId arg) 2258
                "The trailing comma is part of the value, not a separator. De在 POSIX sh 中，未定义 let。e or quote it."
  ...
  where
   ...
   check id f x =
    case oversimplify x of
      ("ls":n) ->
        在 POSIX sh 中，未定义 let。 warntype = if any ("-" `isPrefixOf`) n then warn else err in
          warntype id 2045 "在 ls 输出上进行迭代会比较脆弱。请使用 glob。"
      ("find":_) -> warn id 2044 "在 find 输出上进行 for 循环会比较脆弱。请使用 find -exec 或 while read 循环。"
      _ -> return ()
...
checkFindExec _ cmd@(T_SimpleCommand _ _ t@(h:r)) | cmd `isCommand` "find" = do
    c <- broken r False
    when c $
        在 POSIX sh 中，未定义 let。 wordId = getId $ last t in
  ...
  where
    ...
    warnFor x =
        when(shouldWarn x) $
            info (getId x) 2014 "这将在 find 运行之前仅展开一次，而不是对每个找到的文件展开一次。"
  ...
  where
    ...
    examine t contents =
        unless (null contents || shouldBeSplit t || isQuoteFree (shellType params) tree t || usedAsCommandName tree t) $
            warn (getId t) 2046 "使用引号将其引起来，以防止单词分割 。"
...
checkShorthandIf params x@(T_OrIf _ (T_AndIf id _ _) (T_Pipeline _ _ t))
    ...
    info id 2015 "注意，A && B || C 并非 if-then-else 。A 为 true 时 C 可能会运行。"
  where
...
checkDollarStar p t@(T_NormalWord _ [T_DollarBraced id _ l])
      ...
      在 POSIX sh 中，未定义 let。 str = concat (oversimplify l)
      when ("*" `isPrefixOf` str) $
...
checkConcatenatedDollarAt p word@T_NormalWord {}
  ...
  where
    ...
    for t = err (getId t) 2145 "参数中混合了字符串和数组。请使用 * 或者独立的参数。"
...
checkArrayAsString _ (T_Assignment id _ _ _ word) =
    ...
    then
      warn (getId word) 2124
        "试图给字符串赋值一个数组！请给数组赋值，或者使用 * 代替 @ 来进行连接操作。"
    else
...
prop_checkArrayWithoutIndex3 = verifyTree checkArrayWithoutIndex "在POSIX sh中，coproc 未定义。 foo while true; do echo cow; done; echo $foo"
prop_checkArrayWithoutIndex4 = verifyTree checkArrayWithoutIndex "在POSIX sh中，coproc 未定义。 tail -f log; echo $COPROC"
...
prop_checkArrayWithoutIndex7 = verifyTree checkArrayWithoutIndex "a=(a b); a在 POSIX sh 中，未定义 +=。c"
prop_checkArrayWithoutIndex8 = verifyTree checkArrayWithoutIndex "在 POSIX sh 中，未定义 declare。 -a foo; foo=bar;"
  ...
  where
    ...
    writeF _ (T_Assignment id mode name [] _) _ (DataString _) = do
        ...
        return $ if not isArray then [] else
            case mode of
                Assign -> [makeComment WarningC id 2178 "变量被当做数组使用，但是却给赋值了一个字符串。"]
                Append -> [makeComment WarningC id 2179 "Use array在 POSIX sh 中，未定义 +=。(\"item\") to append items to an array."]
    ...
    writeF _ expr name _ = do
        if isIndexed expr
          ...
          else modify (Map.de在 POSIX sh 中，未定义 let。e name)
        return []
...
lt x = trace ("Tracing " 在 POSIX sh 中，未定义 ++ 。 show x) x -- STRIP
ltt t = trace ("Tracing " 在 POSIX sh 中，未定义 ++ 。 show t)  -- STRIP
...
checkSingleQuotedVariables params t@(T_SingleQuoted id s) =
  ...
  where
    ...
    getFindCommand (T_SimpleCommand _ _ words) =
        在 POSIX sh 中，未定义 let。 list = map getLiteralString words
...
checkUnquotedN _ (TC_Unary _ SingleBracket "-n" t) | willSplit t =
    unless (any isArrayExpansion $ getWordParts t) $ -- There's SC2198 for these
       err (getId t) 2070 "-n 不能与未加引号的参数共同工作。请加引号或使用 在 POSIX sh 中，未定义 [[ ]]。。"
...
prop_checkNumberComparisons25 = verify checkNumberComparisons "[[ $((n在 POSIX sh 中，未定义 ++ 。)) > $x ]]"
...
checkNumberComparisons params (TC_Binary id typ op lhs rhs) = do
    if isNum lhs || isNum rhs
      then do
        when (isLtGt op) $
          err id 2071 $
            op 在 POSIX sh 中，未定义 ++ 。 " is for string comparisons. Use " 在 POSIX sh 中，未定义 ++ 。 eqv op 在 POSIX sh 中，未定义 ++ 。 " instead."
        when (isLeGe op && hasStringComparison) $
            err id 2071 $ op 在 POSIX sh 中，未定义 ++ 。 " is not a valid operator. " 在 POSIX sh 中，未定义 ++ 。
              "Use " 在 POSIX sh 中，未定义 ++ 。 eqv op 在 POSIX sh 中，未定义 ++ 。 " ."
      else do
        when (isLeGe op || isLtGt op) $
            mapM_ checkDecimals [lhs, rhs]

        when (isLeGe op && hasStringComparison) $
            err id 2122 $ op 在 POSIX sh 中，未定义 ++ 。 " is not a valid operator. " 在 POSIX sh 中，未定义 ++ 。
                "Use '! a " 在 POSIX sh 中，未定义 ++ 。 esc 在 POSIX sh 中，未定义 ++ 。 invert op 在 POSIX sh 中，未定义 ++ 。 " b' instead."

        when (typ == SingleBracket && op `elem` ["<", ">"]) $
            case shellType params of
                Sh -> return ()  -- These are unsupported and will be caught by bashism checks.
                Dash -> err id 2073 $ "Escape \\" 在 POSIX sh 中，未定义 ++ 。 op 在 POSIX sh 中，未定义 ++ 。 " to prevent it redirecting."
                BusyboxSh -> err id 2073 $ "Escape \\" 在 POSIX sh 中，未定义 ++ 。 op 在 POSIX sh 中，未定义 ++ 。 " to prevent it redirecting."
                _ -> err id 2073 $ "Escape \\" 在 POSIX sh 中，未定义 ++ 。 op 在 POSIX sh 中，未定义 ++ 。 " to prevent it redirecting (or switch to [[ .. ]])."
  ...
  where
      ...
      decimalError = "Decimals are not supported. " 在 POSIX sh 中，未定义 ++ 。
      ...
      checkString t =
        ...
        in
            when (isNonNum t) $
                ...
                then
                    err (getId t) 2170 $
                        "Invalid number for " 在 POSIX sh 中，未定义 ++ 。 op 在 POSIX sh 中，未定义 ++ 。 ". Use " 在 POSIX sh 中，未定义 ++ 。 seqv op 在 POSIX sh 中，未定义 ++ 。
                        " to compare as string (or use " 在 POSIX sh 中，未定义 ++ 。 fix 在 POSIX sh 中，未定义 ++ 。
                        " to expand as " 在 POSIX sh 中，未定义 ++ 。 kind 在 POSIX sh 中，未定义 ++ 。 ")."
                else
                    ...
                    when (not isVar || any isQuotes (getWordParts t) || asString `notElem` assignedVariables) $
                        warn (getId t) 2309 $
                            op 在 POSIX sh 中，未定义 ++ 。 " treats this as " 在 POSIX sh 中，未定义 ++ 。 kind 在 POSIX sh 中，未定义 ++ 。 ". " 在 POSIX sh 中，未定义 ++ 。
                            "Use " 在 POSIX sh 中，未定义 ++ 。 seqv op 在 POSIX sh 中，未定义 ++ 。 " to compare as string (or expand explicitly with " 在 POSIX sh 中，未定义 ++ 。 fix 在 POSIX sh 中，未定义 ++ 。 ")."
      ...
      seqv "-ge" = "! a " 在 POSIX sh 中，未定义 ++ 。 esc 在 POSIX sh 中，未定义 ++ 。 "< b"
      seqv "-gt" = esc 在 POSIX sh 中，未定义 ++ 。 ">"
      seqv "-le" = "! a " 在 POSIX sh 中，未定义 ++ 。 esc 在 POSIX sh 中，未定义 ++ 。 "> b"
      seqv "-lt" = esc 在 POSIX sh 中，未定义 ++ 。 "<"
      seqv "-eq" = "="
      seqv "-ne" = "!="
...
checkDoubleBracketOperators _ x@(TC_Binary id typ op lhs rhs)
    | typ == DoubleBracket && op `elem` ["\\<", "\\>"] =
        err id 2075 $ "Escaping " 在 POSIX sh 中，未定义 ++ 。 op 在 POSIX sh 中，未定义 ++ 。" is required in [..], but invalid in [[..]]"
...
checkConditionalAndOrs _ t =
    case t of
        (TC_And id SingleBracket "&&" _ _) ->
            err id 2107 "请不要使用[ a && b ]，应使用 [ a ] && [ b ] 。"
        (TC_And id DoubleBracket "-a" _ _) ->
            err id 2108 "在 [[..]]中，请使用 && 代替 -a。"
        (TC_Or id SingleBracket "||" _ _) ->
            err id 2109 "请不要使用 [ a || b ]，应使用 [ a ] || [ b ]。"
        (TC_Or id DoubleBracket "-o" _ _) ->
            err id 2110 "在 [[..]]中，请使用 || 代替 -o。"

        (TC_And id SingleBracket "-a" _ _) ->
            warn id 2166 "请使用 [ p ] && [ q ] ，因为 [ p -a q ] 的定义不明确，结果不确定。"
...
checkGlobbedRegex _ (TC_Binary _ DoubleBracket "=~" _ rhs)
    | isConfusedGlobRegex s =
        warn (getId rhs) 2049 "=~ 用于正则表达式，但这看起来像 glob。请改用 =。"
...
prop_checkConstantIfs7 = verifyNot checkConstantIfs "[ a 在 POSIX sh 中，未定义 -nt。 b ]"
...
checkLiteralBreakingTest _ t = sequence_ $
        case t of
            (TC_Nullary _ _ w@(T_NormalWord _ l)) -> do
                ...
                comparisonWarning l `mplus` tautologyWarning w "由于内容是文字字符串，因此推断 -n 总是会返回真。"
  ...
  where
    ...
    comparisonWarning list = do
        ...
        return $ err (getId token) 2077 "比较运算符前后需要使用空格围起。"
...
checkConstantNullary _ (TC_Nullary _ _ t) | isConstant t =
    case onlyLiteralString t of
        "false" -> err (getId t) 2158 "[ false ] 的结果是真。请删除中括号."
        ...
        _ -> err (getId t) 2078 "此表达式为常量。您是否在某些地方忘了加 $？"
  where
...
prop_checkForDecimals3 = verifyNot checkForDecimals "在 POSIX sh 中，未定义 declare。 -A foo; foo[1.2]=bar"
checkForDecimals params t@(TA_Expansion id _) = sequence_ $ do
    ...
    return $ err id 2079 "(( )) 不支持小数，请使用 bc 或 awk。"
...
checkDivBeforeMult params (TA_Binary _ "*" (TA_Binary id "/" _ x) y)
    | not (hasFloatingPoint params) && x /= y =
        info id 2017 "将 a/b*c 替换为 a*c/b 以提高精度。"
checkDivBeforeMult _ _ = return ()

prop_checkArithmeticDeref = verify checkArithmeticDeref "echo $((3+$foo))"
prop_checkArithmeticDeref2 = verify checkArithmeticDeref "cow=14; (( s在 POSIX sh 中，未定义 +=。 $cow ))"
prop_checkArithmeticDeref3 = verifyNot checkArithmeticDeref "cow=1/40; (( s在 POSIX sh 中，未定义 +=。 ${cow%%/*} ))"
...
prop_checkArithmeticDeref8 = verifyNot checkArithmeticDeref "在 POSIX sh 中，未定义 let。 i=$i+1"
...
prop_checkArithmeticDeref11b = verifyNot checkArithmeticDeref "在 POSIX sh 中，未定义 declare。 -A a; a[$foo]=wee"
...
checkArithmeticDeref params t@(TA_Expansion _ [T_DollarBraced id _ l]) =
  ...
  where
    isException [] = True
    isException s@(h:_) = any (`elem` "/.:#%?*@$-!在 POSIX sh 中，未定义 +=。^,") s || isDigit h
    ...
    normalWarning = style id 2004 "$和${} 在算术变量上非必要。"
    noWarning = return ()
...
checkArithmeticBadOctal _ t@(TA_Expansion id _) = sequence_ $ do
    ...
    return $ err id 2080 "使用0开头的数字降为被处理八进制数字。"
  where
...
checkComparisonAgainstGlob _ (TC_Binary _ DoubleBracket op _ (T_NormalWord id [T_DollarBraced _ _ _]))
    | op `elem` ["=", "==", "!="] =
        warn id 2053 $ "Quote the right-hand side of " 在 POSIX sh 中，未定义 ++ 。 op 在 POSIX sh 中，未定义 ++ 。 " in 在 POSIX sh 中，未定义 [[ ]]。 to prevent glob matching."
  ...
  where
    ...
    check' expr@(T_NormalWord _ list)
        ...
        | not (isGlob expr) && any isQuoteableExpansion list =
            warn (getId expr) 2254 "请使用引号扩展来进行文字匹配而非 glob。"
    check' _ = return ()

prop_checkCommarrays1 = verify checkCommarrays "a=(1, 2)"
prop_checkCommarrays2 = verify checkCommarrays "a在 POSIX sh 中，未定义 +=。(1,2,3)"
...
checkOrNeq _ (TC_Or id typ op (TC_Binary _ _ op1 lhs1 rhs1 ) (TC_Binary _ _ op2 lhs2 rhs2))
    | (op1 == op2 && (op1 == "-ne" || op1 == "!=")) && lhs1 == lhs2 && rhs1 /= rhs2 && not (any isGlob [rhs1,rhs2]) =
        warn id 2055 $ "You probably wanted " 在 POSIX sh 中，未定义 ++ 。 (if typ == SingleBracket then "-a" else "&&") 在 POSIX sh 中，未定义 ++ 。 " here, otherwise it's always true."

-- For arithmetic context "or"
checkOrNeq _ (TA_Binary id "||" (TA_Binary _ "!=" word1 _) (TA_Binary _ "!=" word2 _))
    | word1 == word2 =
        warn id 2056 "您可能需要在此处添加 && ，否则它将一直是真。"
...
checkOrNeq _ (T_OrIf id lhs rhs) = sequence_ $ do
    ...
    return $ warn id 2252 "您可能需要在此处添加 && ，否则它将一直是真。"
  where
    getExpr x =
        case x of
...
prop_checkUuoeVar9 = verify checkUuoeVar "foo $(echo 在 POSIX sh 中，未定义 $(<文件)。)"
checkUuoeVar _ p =
    case p of
...
checkTestRedirects _ (T_Redirecting id redirs cmd) | cmd `isCommand` "test" =
  ...
  where
    check t =
        when (suspicious t) $
            warn (getId t) 2065 "这将被解释为 shell 文件重定向，而非比较。"
...
checkPS1Assignments _ (T_Assignment _ _ "PS1" _ word) = warnFor word
  where
    warnFor word =
        在 POSIX sh 中，未定义 let。 contents = concat $ oversimplify word in
    ...
    containsUnescaped s =
        在 POSIX sh 中，未定义 let。 unenclosed = subRegex enclosedRegex s "" in
...
checkBadParameterSubstitution _ t =
  ...
  where
    ...
    isIndirection vars =
        在 POSIX sh 中，未定义 let。 list = mapMaybe isIndirectionPart vars in
    ...
    checkFirst t =
        case t of
            T_Literal id (c:_) ->
                ...
                then return ()
                else err id 2296 $ "Parameter expansions can't start with " 在 POSIX sh 中，未定义 ++ 。 e4m [c] 在 POSIX sh 中，未定义 ++ 。 ". Double check syntax."
            ...
            T_DollarBraced id braces _ | isUnmodifiedParameterExpansion t ->
                err id 2298 $
                    (if braces then "${${x}}" else "${$x}")
                      在 POSIX sh 中，未定义 ++ 。 " is invalid. For expansion, use ${x}. For indirection, use arrays, ${!x} or (for sh) eval."

            T_DollarBraced {} ->
                err (getId t) 2299 "参数展开不能内嵌。 请使用临时变量。"

            _ | isCommandSubstitution t ->
                err (getId t) 2300 "参数展开不能用来进行命令替换。请使用临时变量。"

            _ -> err (getId t) 2301 $ "Parameter expansion starts with unexpected " 在 POSIX sh 中，未定义 ++ 。 name t 在 POSIX sh 中，未定义 ++ 。 ". Double check syntax."
...
prop_checkInexplicablyUnquoted12 = verifyNot checkInexplicablyUnquoted "在 POSIX sh 中，未定义 declare。 \"foo\"=\"bar\""
checkInexplicablyUnquoted params (T_NormalWord id tokens) = mapM_ check (tails tokens)
  where
    ...
    warnAboutExpansion id =
        warn id 2027 "包围的引号实际取消了引号。将其移除或转义。"
    warnAboutLiteral id =
...
checkTildeInQuotes _ = check
  where
    verify id ('~':'/':_) = warn id 2088 "引号中的波浪符号不会被展开。请使用 $HOME。"
    verify _ _ = return ()
...
checkSshHereDoc _ (T_Redirecting _ redirs cmd)
  ...
  where
    ...
    checkHereDoc (T_FdRedirect _ _ (T_HereDoc id _ Unquoted token tokens))
        ...
        warn id 2087 $ "Quote '" 在 POSIX sh 中，未定义 ++ 。 (e4m token) 在 POSIX sh 中，未定义 ++ 。 "' to make here document expansions happen on the server side rather than on the client."
...
prop_subshellAssignmentCheck7 = verifyTree    subshellAssignmentCheck "cmd | while read foo; do (( n在 POSIX sh 中，未定义 ++ 。 )); done; echo \"$n lines\""
prop_subshellAssignmentCheck8 = verifyTree    subshellAssignmentCheck "n=3 & echo $((n在 POSIX sh 中，未定义 ++ 。))"
...
prop_subshellAssignmentCheck11 = verifyTree subshellAssignmentCheck "cat /etc/passwd | while read line; do 在 POSIX sh 中，未定义 let。 n=n+1; done\necho $n"
prop_subshellAssignmentCheck12 = verifyTree subshellAssignmentCheck "cat /etc/passwd | while read line; do 在 POSIX sh 中，未定义 let。 在 POSIX sh 中，未定义 ++ 。n; done\necho $n"
...
subshellAssignmentCheck params t =
    在 POSIX sh 中，未定义 let。 flow = variableFlow params
...
findSubshelled (Reference (_, readToken, str):rest) scopes deadVars = do
    unless (shouldIgnore str) $ case Map.findWithDefault Alive str deadVars of
        ...
        Dead writeToken reason -> do
                    info (getId writeToken) 2030 $ "Modification of " 在 POSIX sh 中，未定义 ++ 。 str 在 POSIX sh 中，未定义 ++ 。 " is 在 POSIX sh 中，未定义 local。 (to subshell caused by "在 POSIX sh 中，未定义 ++ 。 reason 在 POSIX sh 中，未定义 ++ 。")."
                    info (getId readToken) 2031 $ str 在 POSIX sh 中，未定义 ++ 。 " was modified in a subshell. That change might be lost."
...
doVariableFlowAnalysis readFunc writeFunc empty flow = evalState (
    foldM (\list x -> do { l <- doFlow x;  return $ l 在 POSIX sh 中，未定义 ++ 。 list; }) [] flow
    ) empty
  where
...
prop_checkSpacefulnessCfg15 = verifyNot checkSpacefulnessCfg "在 POSIX sh 中，未定义 local。 foo=$1"
prop_checkSpacefulnessCfg16 = verifyNot checkSpacefulnessCfg "在 POSIX sh 中，未定义 declare。 foo=$1"
...
prop_checkSpacefulnessCfg20 = verifyNot checkSpacefulnessCfg "n在 POSIX sh 中，未定义 +=。'foo bar'"
...
prop_checkSpacefulnessCfg34 = verify checkSpacefulnessCfg "在 POSIX sh 中，未定义 declare。 foo$n=$1"
...
prop_checkSpacefulnessCfg46 = verifyNot checkSpacefulnessCfg "x=0; (( x 在 POSIX sh 中，未定义 +=。 1 )); echo $x"
prop_checkSpacefulnessCfg47 = verifyNot checkSpacefulnessCfg "x=0; (( x-- )); echo $x"
prop_checkSpacefulnessCfg48 = verifyNot checkSpacefulnessCfg "x=0; (( 在 POSIX sh 中，未定义 ++ 。x )); echo $x"
...
prop_checkSpacefulnessCfg53 = verifyNot checkSpacefulnessCfg "s=1; f() { 在 POSIX sh 中，未定义 local。 s='a b'; }; f; echo $s"
...
prop_checkSpacefulnessCfg57 = verifyNot checkSpacefulnessCfg "在 POSIX sh 中，未定义 declare。 -i s=0; s=$(f); echo $s"
prop_checkSpacefulnessCfg58 = verify checkSpacefulnessCfg "f() { 在 POSIX sh 中，未定义 declare。 -i s; }; f; s=$(var); echo $s"
prop_checkSpacefulnessCfg59 = verifyNot checkSpacefulnessCfg "f() { 在 POSIX sh 中，未定义 declare。 -gi s; }; f; s=$(var); echo $s"
prop_checkSpacefulnessCfg60 = verify checkSpacefulnessCfg "在 POSIX sh 中，未定义 declare。 -i s; 在 POSIX sh 中，未定义 declare。 +i s; s=$(foo); echo $s"
prop_checkSpacefulnessCfg61 = verify checkSpacefulnessCfg "在 POSIX sh 中，未定义 declare。 -x X; y=foo$X; echo $y;"
prop_checkSpacefulnessCfg62 = verifyNot checkSpacefulnessCfg "f() { 在 POSIX sh 中，未定义 declare。 -x X; y=foo$X; echo $y; }"
prop_checkSpacefulnessCfg63 = verify checkSpacefulnessCfg "f && 在 POSIX sh 中，未定义 declare。 -i s; s='x + y'; echo $s"
prop_checkSpacefulnessCfg64 = verifyNot checkSpacefulnessCfg "在 POSIX sh 中，未定义 declare。 -i s; s='x + y'; x=$s; echo $x"
...
checkSpacefulnessCfg' dirtyPass params token@(T_DollarBraced id _ list) =
        ...
        unless (name `elem` specialVariablesWithoutSpaces || quotesMayConflictWithSC2281 params token) $
            ...
            then
                ...
                then
                    ...
                    infoWithFix id 2086 "请使用双引号来避免 glob 和单词分割。" $
                        addDoubleQuotesAround params token
            else
                styleWithFix id 2248 "当变量名未包含任何特殊字符时使用双引号。" $
                    addDoubleQuotesAround params token
  ...
  where
    ...
    isDefaultAssignment parents token =
        在 POSIX sh 中，未定义 let。 modifier = getBracedModifier bracedString in
...
checkQuotesInLiterals params t =
  ...
  where
    ...
    de在 POSIX sh 中，未定义 let。eQuotes = modify . Map.de在 POSIX sh 中，未定义 let。e
    parents = parentMap params
    quoteRegex = mkRegex "\"|([/= ]|^)'|'( |$)|\\\\ "
    ...
    writeF _ _ name (DataString (SourceFrom values)) = do
        quoteMap <- get
        在 POSIX sh 中，未定义 let。 quotedVars = msum $ map (forToken quoteMap) values
        case quotedVars of
            Nothing -> de在 POSIX sh 中，未定义 let。eQuotes name
          ...
          Just j
              ...
              -> [
                  makeComment WarningC j 2089 $
                      "Quotes/backslashes will be treated literally. " 在 POSIX sh 中，未定义 ++ 。 suggestion,
  ...
  where
    checkCommand _ t@(T_SimpleCommand _ _ argv) =
        case getCommandNameAndToken False t of
            (Just str, t) -> do
                在 POSIX sh 中，未定义 let。 name = basename str
                在 POSIX sh 中，未定义 let。 args = skipOver t argv
                在 POSIX sh 中，未定义 let。 argStrings = map (\x -> (onlyLiteralString x, x)) args
                在 POSIX sh 中，未定义 let。 candidates = getPotentialCommands name argStrings
    ...
    checkArg cmd cmdId (_, arg) = sequence_ $ do
        ...
        return $ do
            ...
            info definitionId 2032 $
              "This function can't be invoked via " 在 POSIX sh 中，未定义 ++ 。 cmd 在 POSIX sh 中，未定义 ++ 。 patternContext cmdId
...
prop_checkUnused4 = verifyTree checkUnusedAssignments "在 POSIX sh 中，未定义 local。 i=0"
prop_checkUnused5 = verifyNotTree checkUnusedAssignments "read lol; echo $lol"
prop_checkUnused6 = verifyNotTree checkUnusedAssignments "var=4; (( var在 POSIX sh 中，未定义 ++ 。 ))"
...
prop_checkUnused16 = verifyNotTree checkUnusedAssignments "foo=5; 在 POSIX sh 中，未定义 declare。 -x foo"
prop_checkUnused16b = verifyNotTree checkUnusedAssignments "f() { 在 POSIX sh 中，未定义 local。 -x foo; foo=42; bar; }; f"
...
prop_checkUnused19 = verifyNotTree checkUnusedAssignments "a=1; 在 POSIX sh 中，未定义 let。 b=a+1; echo $b"
...
prop_checkUnused26 = verifyNotTree checkUnusedAssignments "在 POSIX sh 中，未定义 declare。 -F foo"
...
prop_checkUnused29 = verifyNotTree checkUnusedAssignments "var=(a b); 在 POSIX sh 中，未定义 declare。 -p var"
prop_checkUnused30 = verifyTree checkUnusedAssignments "在 POSIX sh 中，未定义 let。 a=1"
prop_checkUnused31 = verifyTree checkUnusedAssignments "在 POSIX sh 中，未定义 let。 'a=1'"
prop_checkUnused32 = verifyTree checkUnusedAssignments "在 POSIX sh 中，未定义 let。 a=b=c; echo $a"
...
prop_checkUnused39 = verifyNotTree checkUnusedAssignments "在 POSIX sh 中，未定义 declare。 -x -f foo"
...
prop_checkUnused49 = verifyNotTree checkUnusedAssignments "在 POSIX sh 中，未定义 declare。 -A array; key=a; [[ -v array[$key] ]]"
    ...
    warnFor (name, token) =
        unless ("_" `isPrefixOf` name) $
            warn (getId token) 2034 $
                name 在 POSIX sh 中，未定义 ++ 。 " appears unused. Verify use (or export if used externally)."
...
prop_checkUnassignedReferences5 = verifyNotTree checkUnassignedReferences "在 POSIX sh 中，未定义 declare。 -A foo=([bar]=baz); echo ${foo[bar]}"
...
prop_checkUnassignedReferences8 = verifyNotTree checkUnassignedReferences "在 POSIX sh 中，未定义 let。 'foo = 1'; echo $foo"
...
prop_checkUnassignedReferences11 = verifyNotTree checkUnassignedReferences "在 POSIX sh 中，未定义 declare。 -A foo; echo \"${foo[@]}\""
prop_checkUnassignedReferences12 = verifyNotTree checkUnassignedReferences "typeset -a foo; echo \"${foo[@]}\""
prop_checkUnassignedReferences13 = verifyNotTree checkUnassignedReferences "f() { 在 POSIX sh 中，未定义 local。 foo; echo $foo; }"
...
prop_checkUnassignedReferences16 = verifyNotTree checkUnassignedReferences "在 POSIX sh 中，未定义 declare。 -A foo=( [a b]=bar ); echo ${foo[a b]}"
...
prop_checkUnassignedReferences23 = verifyTree checkUnassignedReferences "在 POSIX sh 中，未定义 declare。 -a foo; foo[bar]=42;"
prop_checkUnassignedReferences24 = verifyNotTree checkUnassignedReferences "在 POSIX sh 中，未定义 declare。 -A foo; foo[bar]=42;"
prop_checkUnassignedReferences25 = verifyNotTree checkUnassignedReferences "在 POSIX sh 中，未定义 declare。 -A foo=(); foo[bar]=42;"
...
prop_checkUnassignedReferences33 = verifyNotTree checkUnassignedReferences "f() { 在 POSIX sh 中，未定义 local。 -A foo; echo \"${foo[@]}\"; }"
prop_checkUnassignedReferences34 = verifyNotTree checkUnassignedReferences "在 POSIX sh 中，未定义 declare。 -A foo; (( foo[bar] ))"
  ...
  where
    ...
    getBestMatch var = do
      ...
      where
        ...
        goodMatch var match score =
            在 POSIX sh 中，未定义 let。 l = length match in
                l > 3 && score <= 1
                || l > 7 && score <= 2
    ...
    warningForGlobals var place = do
        ...
        return $ info (getId place) 2153 $
            "Possible misspelling: " 在 POSIX sh 中，未定义 ++ 。 var 在 POSIX sh 中，未定义 ++ 。 " may not be assigned. Did you mean " 在 POSIX sh 中，未定义 ++ 。 match 在 POSIX sh 中，未定义 ++ 。 "?"

    warningForLocals var place =
        return $ warn (getId place) 2154 $
            var 在 POSIX sh 中，未定义 ++ 。 " is referenced but not assigned" 在 POSIX sh 中，未定义 ++ 。 optionalTip 在 POSIX sh 中，未定义 ++ 。 "."
      where
        optionalTip =
            if var `elem` commonCommands
            then " (for output from commands, use \"$(" 在 POSIX sh 中，未定义 ++ 。 var 在 POSIX sh 中，未定义 ++ 。 " ..." 在 POSIX sh 中，未定义 ++ 。 ")\" )"
            else fromMaybe "" $ do
                    match <- getBestMatch var
                    return $ " (did you mean '" 在 POSIX sh 中，未定义 ++ 。 match 在 POSIX sh 中，未定义 ++ 。 "'?)"
...
checkWhileReadPitfalls params (T_WhileExpression id [command] contents)
  ...
  where
    ...
    isStdinReadCommand (T_Pipeline _ _ [T_Redirecting id redirs cmd]) =
        在 POSIX sh 中，未定义 let。 plaintext = oversimplify cmd
    ...
    checkMuncher (T_Pipeline _ _ (T_Redirecting _ redirs cmd:_)) = do
        ...
        case cmd of
            T_SimpleCommand _ vars args ->
                mapM_ checkMuncher $ concat $ concatMap getCommandSequences $ concatMap getWords $ vars 在 POSIX sh 中，未定义 ++ 。 args
            _ -> return ()
        ...
        unless (any stdinRedirect redirs) $ do
            ...
            sequence_ $ do
                ...
                return $ do
                    info id 2095 $
                        name 在 POSIX sh 中，未定义 ++ 。 " may swallow stdin, preventing this loop from working properly."
                    warnWithFix (getId cmd) 2095
                        ("Use " 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 " " 在 POSIX sh 中，未定义 ++ 。 flag 在 POSIX sh 中，未定义 ++ 。 " to prevent " 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 " from swallowing stdin.")
                        (fix flag cmd)
...
checkPrefixAssignmentReference params t@(T_DollarBraced id _ value) =
  ...
  where
    ...
    checkVar (T_Assignment aId mode aName [] value) |
        ...
        warn aId 2097 "此赋值仅能被复刻的进程看到。"
        warn id 2098 "此展开将无法看到提到的赋值。"
    checkVar _ = return ()
...
checkCharRangeGlob p t@(T_Glob id str) |
  isCharClass str && not isIgnoredCommand && not (isDereferenced t) =
    if ":" `isPrefixOf` contents
        && ":" `isSuffixOf` contents
        && contents /= ":"
    then warn id 2101 "被命名的类外部需要包裹 []， 例如 [[:digit:]]。"
    else
        when ('[' `notElem` contents && hasDupes) $
            info id 2102 "范围只能匹配单个字符(会因为重复提及)。"
  where
...
checkCdAndBack params t =
  ...
  where
    ...
    doList list = sequence_ $ do
        ...
        return $ info cd 2103 "使用 ( subshell ) 来避免必须 cd 回来。"
...
checkLoopKeywordScope params t |
    ...
    then case map subshellType $ filter (not . isFunction) path of
        Just str:_ -> warn (getId t) 2106 $
            "This only exits the subshell caused by the " 在 POSIX sh 中，未定义 ++ 。 str 在 POSIX sh 中，未定义 ++ 。 "."
        _ -> return ()
    else case path of
        -- breaking at a source/function invocation is an abomination. Let's ignore it.
        h:_ | isFunction h -> err (getId t) 2104 $ "In functions, use return instead of " 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 "."
        _ -> err (getId t) 2105 $ name 在 POSIX sh 中，未定义 ++ 。 " is only valid in loops."
  where
    path = 在 POSIX sh 中，未定义 let。 p = getPath (parentMap params) t in NE.filter relevant p
...
checkFunctionDeclarations params
    ...
    where
        forSh = do
            when (hasKeyword && hasParens) $
                warn id 2112 "'function' keyword is non-standard. De在 POSIX sh 中，未定义 let。e it."
...
checkUnpassedInFunctions params root =
  ...
  where
    ...
    updateWith x@(name, _, _) = Map.insertWith (在 POSIX sh 中，未定义 ++ 。) name [x]
    ...
    suggestParams (name, _, thing) =
        info (getId thing) 2119 $
            "Use " 在 POSIX sh 中，未定义 ++ 。 (e4m name) 在 POSIX sh 中，未定义 ++ 。 " \"$@\" if function's $1 should mean script's $1."
    warnForDeclaration func name =
        warn (getId func) 2120 $
            name 在 POSIX sh 中，未定义 ++ 。 " references arguments, but none are ever passed."
...
checkOverridingPath _ (T_SimpleCommand _ vars []) =
  ...
  where
    ...
    checkVar _ = return ()
    notify id = warn id 2123 "PATH 是被 shell 所使用的搜索路径。请另外选一个名字。"
  ...
  where
    checkVar (T_Assignment id Assign "PATH" [] (T_NormalWord _ parts))
        | any (\x -> isQuoted x && hasTilde x) parts =
            warn id 2147 "PATH 中的~在不同程序中工作起来效果很差(不一致)。"
    checkVar _ = return ()
...
checkUnsupported params t =
 ...
 where
    ...
    report s = err (getId t) 2127 $
        "To use " 在 POSIX sh 中，未定义 ++ 。 s 在 POSIX sh 中，未定义 ++ 。 ", specify #!/usr/bin/env " 在 POSIX sh 中，未定义 ++ 。
...
checkMultipleAppends params t =
  ...
  where
    ...
    checkGroup (Just (_,id):_:_:_) =
        style id 2129
            "考虑使用 { cmd1; cmd2; } >> file 来代替多个单重定向语句。"
    checkGroup _ = return ()
  ...
  where
    ...
    check value =
        case value of
            ...
            x | 'n' `elem` x -> suggest2 "the literal 在 POSIX sh 中，未定义 let。ter 'n'"
            x | 't' `elem` x -> suggest2 "the literal 在 POSIX sh 中，未定义 let。ter 't'"
            _ -> return ()
    suggest r = warn (getId value) 2141 $ "This backslash is literal. Did you mean IFS=" 在 POSIX sh 中，未定义 ++ 。 r 在 POSIX sh 中，未定义 ++ 。 " ?"
    suggest2 desc = warn (getId value) 2141 $ "This IFS value contains " 在 POSIX sh 中，未定义 ++ 。 desc 在 POSIX sh 中，未定义 ++ 。 ". For tabs/linefeeds/escapes, use $'..', literal, or printf."
...
checkShouldUseGrepQ params t =
  ...
  where
    check id bool token = do
        ...
        在 POSIX sh 中，未定义 let。 op = if bool then "-n" else "-z"
        在 POSIX sh 中，未定义 let。 flip = if bool then "" else "! "
        return . style id 2143 $
            "Use " 在 POSIX sh 中，未定义 ++ 。 flip 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 " -q instead of " 在 POSIX sh 中，未定义 ++ 。
                "comparing output with [ " 在 POSIX sh 中，未定义 ++ 。 op 在 POSIX sh 中，未定义 ++ 。 " .. ]."
            ...
            then
                when (typ == SingleBracket) $
                    err (getId token) 2208 $
                      "使用 在 POSIX sh 中，未定义 [[ ]]。 或者引号来避免 -v 参数发生 glob 展开。"
            else
                ...
                then
                    ...
                    when (op `elem` [['-', c] | c <- "bcdfgkprsuwxLhNOGRS" ]) $
                        warn (getId token) 2245 $
                            op 在 POSIX sh 中，未定义 ++ 。 " only applies to the first expansion of this glob. Use a loop to check any/all."
                else
                    err (getId token) 2144 $
                       op 在 POSIX sh 中，未定义 ++ 。 " doesn't work with globs. Use a for loop."
  ...
  where
    ...
    checkArrays typ token =
        when (any isArrayExpansion $ getWordParts token) $
            if typ == SingleBracket
            then warn (getId token) 2198 "数组 [ ] 中的运算对象无法工作。请使用循环(或者使用 * 代替 @ 来进行连接)。"
            else err (getId token) 2199 "数组在 POSIX sh 中，未定义 [[ ]]。中会隐式的执行连接操作。请使用循环(或者使用 * 代替 @ 来进行连接)。"

    checkBraces typ token =
        when (any isBraceExpansion $ getWordParts token) $
            if typ == SingleBracket
            then warn (getId token) 2200 "[ ] 中的花括号展开不能工作。请使用循环。"
            else err (getId token) 2201 "在 POSIX sh 中，未定义 [[ ]]。中的花括号展开不会发生。请使用循环。"

    checkGlobs typ token =
        when (isGlob token) $
            if typ == SingleBracket
            then warn (getId token) 2202 "[ ] 中的 glob 不能工作。请使用循环。"
            else err (getId token) 2203 "在 POSIX sh 中，未定义 [[ ]]。中的 glob 被忽略了，除非放在 =，!=右侧。请使用循环。"

    checkNumericalGlob SingleBracket token =
        -- var[x] and x*2 look like globs
        when (shellType params /= Ksh && isGlob token) $
            err (getId token) 2255 "[ ] 不能用于算术运算。使用 $((..)) 来计算数字，或者使用字符串比较符来比较字符串。"
...
checkReadWithoutR _ t@T_SimpleCommand {} | t `isUnqualifiedCommand` "read"
    && "r" `notElem` map snd flags && not has_t0 =
        info (getId $ getCommandTokenOrThis t) 2162 "read 不结合 -r 使用会干扰反斜线。"
  where
    flags = getAllFlags t
    has_t0 = Just "0" == do
        ...
        | name `elem` ["cd", "pushd", "popd"]
            ...
            && not (isCondition $ getPath (parentMap params) t) =
                warnWithFix (getId t) 2164
                    ("Use '" 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 " ... || exit' or '" 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 " ... || return' in case " 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 " fails.")
...
prop_checkLoopVariableReassignment2 = verify checkLoopVariableReassignment "for i in *; do for((i=0; i<3; i在 POSIX sh 中，未定义 ++ 。)); do true; done; done"
...
checkLoopVariableReassignment params token =
  ...
  where
    check = do
        ...
        return $ do
            warn (getId token) 2165 "此嵌套循环内部会覆盖父循环中的索引变量。"
            warn (getId next)  2167 "此父循环的索引变量已经被覆盖。"
...
checkTrailingBracket _ token =
  ...
  where
    check (T_NormalWord id [T_Literal _ str]) command
        ...
        = warn id 2171 $
            "Found trailing " 在 POSIX sh 中，未定义 ++ 。 str 在 POSIX sh 中，未定义 ++ 。 " outside test. Add missing " 在 POSIX sh 中，未定义 ++ 。 opposite 在 POSIX sh 中，未定义 ++ 。 " or quote if intentional."
        where
            opposite = invert str
...
checkReturnAgainstZero params token =
  ...
  where
    ...
    isFirstCommandInFunction = fromMaybe False $ do
        在 POSIX sh 中，未定义 let。 path = getPath (parentMap params) token
    ...
    message forSuccess id = when (isOnlyTestInCommand token && not isFirstCommandInFunction) $ style id 2181 $
        "Check exit code directly with e.g. 'if " 在 POSIX sh 中，未定义 ++ 。 (if forSuccess then "" else "! ") 在 POSIX sh 中，未定义 ++ 。 "mycmd;', not indirectly with $?."
...
checkRedirectedNowhere params token =
    case token of
        ...
        T_Pipeline _ _ list -> forM_ list $ \x -> sequence_ $ do
            ...
            return $ err (getId redir) 2189 "您不能在这个重定向和它要应用的命令中间放置 | 符号。"
...
prop_checkArrayAssignmentIndices1 = verifyTree checkArrayAssignmentIndices "在 POSIX sh 中，未定义 declare。 -A foo; foo=(bar)"
prop_checkArrayAssignmentIndices2 = verifyNotTree checkArrayAssignmentIndices "在 POSIX sh 中，未定义 declare。 -a foo; foo=(bar)"
prop_checkArrayAssignmentIndices3 = verifyNotTree checkArrayAssignmentIndices "在 POSIX sh 中，未定义 declare。 -A foo; foo=([i]=bar)"
prop_checkArrayAssignmentIndices4 = verifyTree checkArrayAssignmentIndices "typeset -A foo; foo在 POSIX sh 中，未定义 +=。(bar)"
...
prop_checkArrayAssignmentIndices10 = verifyTree checkArrayAssignmentIndices "在 POSIX sh 中，未定义 declare。 -A arr; arr=( var=value )"
    ...
    check _ t =
        case t of
            T_Assignment _ _ name [] (T_Array _ list) ->
                在 POSIX sh 中，未定义 let。 isAssoc = name `elem` assocs in
    ...
    checkElement isAssociative t =
        case t of
            ...
            T_NormalWord _ parts ->
                在 POSIX sh 中，未定义 let。 literalEquals = do
                    T_Literal id str <- parts
                    在 POSIX sh 中，未定义 let。 (before, after) = break ('=' ==) str
                    guard $ all isDigit before && not (null after)
                    return $ warnWithFix id 2191 "这里的 = 是文字。要想给索引赋值，请使用不带空格的 ( [index]=value ) 。要想保留为文字，请用引号引起。" (surroundWith id params "\"")
                in
                    if null literalEquals && isAssociative
                    then warn (getId t) 2190 "关联数组中的元素需要索引键，例如:array=( [index]=value ) 。"
...
checkUnmatchableCases params t =
    case t of
        T_CaseExpression _ word list -> do
            ...
            在 POSIX sh 中，未定义 let。 allpatterns  = concatMap snd3 list
            -- Check only the non-fallthrough branches for shadowing
            在 POSIX sh 中，未定义 let。 breakpatterns = concatMap snd3 $ filter (\x -> fst3 x == CaseBreak) list
            ...
            在 POSIX sh 中，未定义 let。 exactGlobs = tupMap wordToExactPseudoGlob breakpatterns
            在 POSIX sh 中，未定义 let。 fuzzyGlobs = tupMap wordToPseudoGlob breakpatterns
            在 POSIX sh 中，未定义 let。 dominators = zip exactGlobs (tails $ drop 1 fuzzyGlobs)
  ...
  where
    ...
    checkParams id first second = do
        when (maybe False (`elem` unaryTestOps) $ getLiteralString first) $
            err id 2204 "(..) 是 subshell。您是指 [ .. ]，一个测试表达式吗？"
        when (maybe False (`elem` binaryTestOps) $ getLiteralString second) $
            warn id 2205 "(..) 是 subshell。您是指 [ .. ]，一个测试表达式吗？"
  ...
  where
    ...
    checkPart part = case part of
        ...
        T_DollarBraced id _ str |
            ...
            -> warn id 2206 $
                ...
                else "使用引号来阻止单词分割或glob，或者用更健壮的分割方式:mapfile 或 read -a。"
        _ -> return ()
    ...
    forCommand id =
        warn id 2207 $
            ...
            else "首选 mapfile 或者 read -a 来分割命令输出(或者使用引号来避免分割)。"
...
checkRedirectionToNumber _ t = case t of
    T_IoFile id _ word -> sequence_ $ do
        ...
        return $ warn id 2210 "这是一个文件重定向。是否是一个比较或者 fd 操作？"
    _ -> return ()
...
checkGlobAsCommand _ t = case t of
    T_SimpleCommand _ _ (first:_)
        | isGlob first ->
            warn (getId first) 2211 "这是一个作为命令名称的 glob 。您是否期望它是 ${..}，数组，或者缺少了引号包围？"
    _ -> return ()
...
checkFlagAsCommand _ t = case t of
    T_SimpleCommand _ [] (first:_)
        | isUnquotedFlag first ->
            warn (getId first) 2215 "这个标记被用作了命令名。换行错误或者忘记了 [ .. ]？"
    _ -> return ()
...
prop_checkPipeToNowhere14 = verify checkPipeToNowhere "ls > foo 在 POSIX sh 中，未定义 &>。 bar"
...
checkPipeToNowhere params t =
  ...
  where
    checkPipe (input, stage, output) = do
        在 POSIX sh 中，未定义 let。 hasConsumers = hasAdditionalConsumers stage
        在 POSIX sh 中，未定义 let。 hasProducers = hasAdditionalProducers stage
        ...
        sequence_ $ do
            ...
            在 POSIX sh 中，未定义 let。 suggestion =
                    if name == "echo"
                    then "Did you want 'cat' instead?"
                    else "Wrong command or missing xargs?"
            return $ warn (getId cmd) 2216 $
                "Piping to '" 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 "', a command that doesn't read stdin. " 在 POSIX sh 中，未定义 ++ 。 suggestion

        sequence_ $ do
            T_Redirecting _ redirs cmd <- return stage
            fds <- mapM getRedirectionFds redirs

            在 POSIX sh 中，未定义 let。 fdAndToken :: [(Integer, Token)]
                fdAndToken =
                  concatMap (\(list, redir) -> map (\n -> (n, redir)) list) $
                    zip fds redirs

            在 POSIX sh 中，未定义 let。 fdMap =
                  Map.fromListWith (在 POSIX sh 中，未定义 ++ 。) $
                    map (\(a,b) -> (a,[b])) fdAndToken

            在 POSIX sh 中，未定义 let。 inputWarning = sequence_ $ do
            ...
            在 POSIX sh 中，未定义 let。 outputWarning = sequence_ $ do
    ...
    warnAboutDupes (n, list@(_:_:_)) =
        forM_ list $ \c -> err (getOpId c) 2261 $
            "Multiple redirections compete for " 在 POSIX sh 中，未定义 ++ 。 str n 在 POSIX sh 中，未定义 ++ 。 ". Use cat, tee, or pass filenames instead."
    ...
    checkRedir cmd = sequence_ $ do
        ...
        return $ warn (getId cmd) 2217 $
            "Redirecting to '" 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 "', a command that doesn't read stdin. " 在 POSIX sh 中，未定义 ++ 。 suggestion
    ...
    commandsWithContext pipes cmds =
        在 POSIX sh 中，未定义 let。 pipeTypes = map pipeType pipes
            inputs = NoPipe : pipeTypes
            outputs = pipeTypes 在 POSIX sh 中，未定义 ++ 。 [NoPipe]
        in
...
checkUseBeforeDefinition _ t =
  ...
  where
    ...
    recursiveSequences x =
        在 POSIX sh 中，未定义 let。 list = concat $ getCommandSequences x in
            if null list
            then [x]
...
prop_checkSubshelledTests6 = verifyNot checkSubshelledTests "( [[ $((i在 POSIX sh 中，未定义 ++ 。)) = 10 ]] )"
prop_checkSubshelledTests7 = verifyNot checkSubshelledTests "( [[ $((i在 POSIX sh 中，未定义 +=。1)) = 10 ]] )"
  ...
  where
    ...
    guardNotAssignment t =
        case t of
            ...
            TA_Unary _ s _ -> guard . not $ "在 POSIX sh 中，未定义 ++ 。" `isInfixOf` s || "--" `isInfixOf` s
            T_DollarBraced _ _ l ->
                在 POSIX sh 中，未定义 let。 str = concat $ oversimplify l
    ...
    case t of
        T_IoFile _ _ (T_NormalWord id [T_Literal _ str]) | str `elem` commonCommands
            && str /= "file" -> -- This would be confusing
                warn id 2238 "请从命令名重定向自或者重定向到而不是对文件。您是否想要 管道/xargs(或者加引号来忽略)？"
        _ -> return ()
        ...
        TC_Nullary _ _ word ->
            case getWordParts word of
                [t] | isCommandSubstitution t ->
                    styleWithFix id 2243 "首选使用 -n 来检察输出(或者不包含 [、[[ 来运行命令以检查执行成功)." fix

                -- If they're constant, you get SC2157 &co
                x | all (not . isConstant) x ->
                    styleWithFix id 2244 "首选使用 -n 来检察输出非空字符串(或用 =，-ne 来检察布尔值或整数)。" fix
                _ -> return ()
            where
                id = getId word
    ...
    case t of
        T_CaseExpression id _ list ->
            unless (any canMatchAny list) $
                info id 2249 "请考虑添加默认的 *) case，即使它只是因为错误退出。"
        _ -> return ()
  where
  ...
  where
    ...
    getNonReturningCommands t =
        case t of
            ...
            T_WhileExpression _ conds cmds -> dropLast conds 在 POSIX sh 中，未定义 ++ 。 cmds
            T_UntilExpression _ conds cmds -> dropLast conds 在 POSIX sh 中，未定义 ++ 。 cmds
            ...
            T_IfExpression _ conds elses ->
                concatMap (dropLast . fst) conds 在 POSIX sh 中，未定义 ++ 。 concatMap snd conds 在 POSIX sh 中，未定义 ++ 。 elses
            _ -> []
...
prop_checkModifiedArithmeticInRedirection1 = verify checkModifiedArithmeticInRedirection "ls > $((i在 POSIX sh 中，未定义 ++ 。))"
prop_checkModifiedArithmeticInRedirection2 = verify checkModifiedArithmeticInRedirection "cat < \"foo$((i在 POSIX sh 中，未定义 ++ 。)).txt\""
prop_checkModifiedArithmeticInRedirection3 = verifyNot checkModifiedArithmeticInRedirection "while true; do true; done > $((i在 POSIX sh 中，未定义 ++ 。))"
prop_checkModifiedArithmeticInRedirection4 = verify checkModifiedArithmeticInRedirection "cat <<< $((i在 POSIX sh 中，未定义 ++ 。))"
...
checkAliasUsedInSameParsingUnit params root =
  ...
  where
    lineSpan t =
        在 POSIX sh 中，未定义 let。 m = tokenPositions params in do
                        ...
                        case cmd of
                            Just alias ->
                                unless (isSourced params t || shouldIgnoreCode params 2262 alias) $ do
                                    warn (getId alias) 2262 "别名不能在同一个解析单元中定义和使用。请使用函数来代替。"
                                    info (getId t) 2263 "因为它们位于同一个解析单一，此命令不会引用到之前提到的别名。"
                            _ -> return ()
                    _ -> return ()
            _ -> return ()
    addAlias arg = do
        在 POSIX sh 中，未定义 let。 (name, value) = break (== '=') $ getLiteralStringDef "-" arg
...
checkBlatantRecursion params t =
  ...
  where
    ...
    checkCommand name cmd = sequence_ $ do
        在 POSIX sh 中，未定义 let。 (invokedM, t) = getCommandNameAndToken True cmd
        invoked <- invokedM
        guard $ name == invoked
        return $
...
checkBadTestAndOr params t =
  ...
  where
    checkOrs seps cmds =
        在 POSIX sh 中，未定义 let。 maybeSeps = map Just seps
            commandWithSeps = zip3 (Nothing:maybeSeps) cmds (maybeSeps 在 POSIX sh 中，未定义 ++ 。 [Nothing])
        in
    ...
    checkAnds id t =
        case t of
            ...
            cmd -> when (isTest cmd) $
                errWithFix id 2265 "请使用&& 来实现逻辑与。单个&将会后置并返回true。" $
                    (fixWith [replaceEnd id params 0 "&"])
         ...
         case getWordParts token of
            T_Literal id ('x':_):_ ->
                case token of
                    ...
                    -- Otherwise we can just de在 POSIX sh 中，未定义 let。e it
        ...
        case t of
            T_Assignment id Assign name [] t ->
                case getWordParts t of
                    ...
                    _ -> return ()
            _ -> return ()
    msg id = info id 2269 "自变量被赋值给了自身，因此赋值将不会产生任何效果。"
...
prop_checkEqualsInCommand5f = verifyCodes checkEqualsInCommand [2281] "$var在 POSIX sh 中，未定义 +=。foo"
...
checkEqualsInCommand params originalToken =
  ...
  where
    ...
    indirectionMsg id =
        err id 2271 "For indirection, use arrays, 在 POSIX sh 中，未定义 declare。 \"var$n=value\", or (for sh) read/eval."
    badComparisonMsg id =
        err id 2272 "Command name contains ==. For comparison, use [ \"$var\" = value ]."
    conflictMarkerMsg id =
        err id 2273 "发现 === 序列。代码合并冲突或者是想用来表示注释分界？"
    borderMsg id =
        err id 2274 "命令名以 === 开始。想用来表示注释分界？"
    prefixMsg id =
        err id 2275 "命令名以 =开始。换行错误？"
    genericMsg id =
        err id 2276 "This is interpreted as a command name containing '='. Bad assignment or comparison?"
    assign0Msg id bashfix =
        case shellType params of
            Bash -> errWithFix id 2277 "在 bash 中使用 BASH_ARGV0 来给 $0 赋值(或者使用 [ ]进行比较)。" bashfix
            Ksh -> err id 2278 "$0 无法在 Ksh 中被赋值(但是它的确映射为当前的函数名)。"
            Dash -> err id 2279 "$0 无法在 Dash 中被赋值。这将变成一个命令名。"
            BusyboxSh -> err id 2279 "$0 can't be assigned in Busybox Ash. This becomes a command name."
            _ -> err id 2280 "$0 无法使用此方式赋值，而且当前尚无其它通用的替代方式。"
    leadingNumberMsg id =
        err id 2282 "变量名不能使用数字开始，这将被解释为命令。"
    ...
    msg cmd leading (T_Literal litId s) = do
        ...
        case leading of
            ...
            [T_DollarBraced id braced l] | "=" `isPrefixOf` s -> do
                在 POSIX sh 中，未定义 let。 variableStr = concat $ oversimplify l
                在 POSIX sh 中，未定义 let。 variableReference = getBracedReference variableStr
                在 POSIX sh 中，未定义 let。 variableModifier = getBracedModifier variableStr
                在 POSIX sh 中，未定义 let。 isPlain = isVariableName variableStr
                在 POSIX sh 中，未定义 let。 isPositional = all isDigit variableStr

                在 POSIX sh 中，未定义 let。 isArray = variableReference /= ""
                ...
                case () of
                    ...
                    _ | isArray || isPlain ->
                        errWithFix id 2281
                            ("Don't use " 在 POSIX sh 中，未定义 ++ 。 (if braced then "${}" else "$") 在 POSIX sh 中，未定义 ++ 。 " on the left side of assignments.") $
                                fixWith $
                                    if braced
...
prop_checkSecondArgIsComparison9 = verify checkSecondArgIsComparison "var 在 POSIX sh 中，未定义 +=。 $(foo)"
prop_checkSecondArgIsComparison10 = verify checkSecondArgIsComparison "var 在 POSIX sh 中，未定义 +=。$(foo)"
            ...
            case argString of
                ...
                '+':'=':_ ->
                        return $ err (headId t) 2285 $
                            "删除 在 POSIX sh 中，未定义 +=。 号周围的空格来进行赋值(或者引起 '在 POSIX sh 中，未定义 +=。' 来表示文字)。"
                '=':'=':_ ->
                        return $ err (getId t) 2284 $
                            "使用 [ x = y ] 来比较值(或者引起 '=' 来表示文字)。"
                '=':_ ->
...
checkCommandWithTrailingSymbol _ t =
    case t of
        T_SimpleCommand _ _ (cmd:_) ->
            在 POSIX sh 中，未定义 let。 str = getLiteralStringDef "x" cmd
            ...
            in
                case str of
                    ...
                    "" -> err (getId cmd) 2286 "此空字符串被解释为了命令名。请再次检查语法(或使用 true 作为无操作)。"
                    _ | last == '/' -> err (getId cmd) 2287 "这将被解释为以 '/' 结尾的命令名。请再次检查语法。"
                    _ | last `elem` "\\.,([{<>}])#\"\'% " -> warn (getId cmd) 2288 ("This is interpreted as a command name ending with " 在 POSIX sh 中，未定义 ++ 。 (format last) 在 POSIX sh 中，未定义 ++ 。 ". Double check syntax.")
                    _ | '\t' `elem` str -> err (getId cmd) 2289 "This is interpreted as a command name containing a tab. Double check syntax."
                    _ | '\n' `elem` str -> err (getId cmd) 2289 "这将被解释为包含换行符的命令名。请再次检查语法。"
                    _ -> return ()
        _ -> return ()
  where
    format x =
  ...
  where
    check _ t = case t of
        T_Condition id SingleBracket _ ->
            styleWithFix id 2292 "在 Bash 或 Ksh 中首选使用 在 POSIX sh 中，未定义 [[ ]]。 代替 [ ] 进行测试。" (fixFor t)
        _ -> return ()
...
checkUnquotedParameterExpansionPattern params x =
    case x of
        T_DollarBraced _ True word@(T_NormalWord _ (T_Literal _ s : rest@(_:_))) -> do
            在 POSIX sh 中，未定义 let。 modifier = getBracedModifier $ concat $ oversimplify word
...
checkArrayValueUsedAsIndex params _ =
  ...
  where
    ...
    write _ _ name _ = do
        modify $ Map.de在 POSIX sh 中，未定义 let。e name
        return []
    ...
    read _ t name = do
        ...
        return $ fromMaybe [] $ do
            ...
            在 POSIX sh 中，未定义 let。 loopId = getId loop
            ...
            return [
                ...
                makeComment WarningC (getId arrayRef) 2303 $ (e4m name) 在 POSIX sh 中，未定义 ++ 。 " is an array value, not a key. Use directly or loop over keys instead."
                ]
    ...
    getArrayIfUsedAsIndex name t =
        case t of
            T_DollarBraced _ _ list -> do
                在 POSIX sh 中，未定义 let。 ref = getBracedReference $ concat $ oversimplify list
                guard $ ref == name
                ...
                在 POSIX sh 中，未定义 let。 str = concat $ oversimplify list
                在 POSIX sh 中，未定义 let。 modifier = getBracedModifier str
            ...
            T_NormalWord wordId list -> do
                    ...
                    在 POSIX sh 中，未定义 let。 str = concat $ oversimplify t
                    在 POSIX sh 中，未定义 let。 modifier = getBracedModifier str
                    guard $ ("[" 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 "]") `isPrefixOf` modifier
    ...
    checkCmd cmd = go $ NE.toList $ getPath (parentMap params) cmd
      where
        ...
        informConditional condType t =
            info (getId t) 2310 (
                "This function is invoked in " 在 POSIX sh 中，未定义 ++ 。 condType 在 POSIX sh 中，未定义 ++ 。 " so set -e " 在 POSIX sh 中，未定义 ++ 。
                "will be disabled. Invoke separately if failures should " 在 POSIX sh 中，未定义 ++ 。
                "cause the script to exit.")
        informUninherited t =
            info (getId t) 2311 (
                "Bash implicitly disabled set -e for this function " 在 POSIX sh 中，未定义 ++ 。
                "invocation because it's inside a command substitution. " 在 POSIX sh 中，未定义 ++ 。
...
prop_checkExtraMaskedReturns35 = verifyTree checkExtraMaskedReturns "f() { 在 POSIX sh 中，未定义 local。 -r x=$(false); }"
...
checkExtraMaskedReturns params t =
  ...
  where
    ...
    findMaskingNodes _ (T_SimpleCommand _ assigns (_:args)) = findMaskedNodesInList $ assigns 在 POSIX sh 中，未定义 ++ 。 args
    ...
    inform t = info (getId t) 2312 ("Consider invoking this command "
        在 POSIX sh 中，未定义 ++ 。 "separately to avoid masking its return value (or use '|| true' "
        在 POSIX sh 中，未定义 ++ 。 "to ignore).")
    ...
    isCheckedElsewhere t = any isDeclaringCommand $ NE.tail $ parents params t
      where
        isDeclaringCommand t = fromMaybe False $ do
            ...
            return $
                case basename of
                    -- 在 POSIX sh 中，未定义 local。 -r x=$(false) is intentionally ignored for SC2155
                    "在 POSIX sh 中，未定义 local。" | "r" `elem` (map snd $ getAllFlags cmd) -> False
...
checkCommandIsUnreachable params t =
    case t of
        T_Pipeline {} -> sequence_ $ do
            ...
            return $ info id 2317 "命令似乎不可触达。请检查用法（或者间接调用时忽略）。"
        _ -> return ()
  where id = getId t
...
checkOverwrittenExitCode params t =
  ...
  where
    check id = sequence_ $ do
        cfga <- cfgAnalysis params
        state <- CF.getIncomingState cfga id
        在 POSIX sh 中，未定义 let。 exitCodeIds = CF.exitCodes state
        guard . not $ S.null exitCodeIds

        在 POSIX sh 中，未定义 let。 idToToken = idMap params
        ...
        return $ do
            when (all isCondition exitCodeTokens && not (usedUnconditionally cfga t exitCodeIds)) $
                warn id 2319 "$? 引用到一个条件，但不是一个命令。赋值给变量来避免其被覆盖。"
            when (all isPrinting exitCodeTokens) $
                warn id 2320 "$? 引用到echo/printf，并非上一个命令。赋值到变量来避免它被覆盖掉。"
...
prop_checkUnnecessaryArithmeticExpansionIndex4 = verifyNot checkUnnecessaryArithmeticExpansionIndex "在 POSIX sh 中，未定义 declare。 -A a; a[$((1+1))]=val"
checkUnnecessaryArithmeticExpansionIndex params t =
    case t of
        T_Assignment _ mode var [TA_Sequence _ [ TA_Expansion _ [expansion@(T_DollarArithmetic id _)]]] val ->
            styleWithFix id 2321 "数组下标已经是算术上下文。建议删除 $(( 和))。" $ fix id
        _ -> return ()
...
checkUnnecessaryParens params t =
    case t of
        ...
        TA_Parentesis _ (TA_Sequence _ [ TA_Parentesis id _ ]) ->
            styleWithFix id 2322 "在算术上下文中， ((x)) 和(x)相同。建议只留下一层圆括号。" $ fix id
        _ -> return ()
  where

    checkLeading str t =
        case t of
            TA_Sequence _ [TA_Parentesis id _ ] -> styleWithFix id 2323 (str 在 POSIX sh 中，未定义 ++ 。 ". Prefer not wrapping in additional parentheses.") $ fix id
            _ -> return ()
...
prop_checkPlusEqualsNumber1 = verify checkPlusEqualsNumber "x在 POSIX sh 中，未定义 +=。1"
prop_checkPlusEqualsNumber2 = verify checkPlusEqualsNumber "x在 POSIX sh 中，未定义 +=。42"
prop_checkPlusEqualsNumber3 = verifyNot checkPlusEqualsNumber "(( x 在 POSIX sh 中，未定义 +=。 1 ))"
prop_checkPlusEqualsNumber4 = verifyNot checkPlusEqualsNumber "在 POSIX sh 中，未定义 declare。 -i x=0; x在 POSIX sh 中，未定义 +=。1"
prop_checkPlusEqualsNumber5 = verifyNot checkPlusEqualsNumber "x在 POSIX sh 中，未定义 +=。'1'"
prop_checkPlusEqualsNumber6 = verifyNot checkPlusEqualsNumber "n=foo; x在 POSIX sh 中，未定义 +=。n"
prop_checkPlusEqualsNumber7 = verify checkPlusEqualsNumber "n=4; x在 POSIX sh 中，未定义 +=。n"
prop_checkPlusEqualsNumber8 = verify checkPlusEqualsNumber "n=4; x在 POSIX sh 中，未定义 +=。$n"
prop_checkPlusEqualsNumber9 = verifyNot checkPlusEqualsNumber "在 POSIX sh 中，未定义 declare。 -ia var; var[x]在 POSIX sh 中，未定义 +=。1"
checkPlusEqualsNumber params t =
    case t of
        T_Assignment id Append var _ word -> sequence_ $ do
            ...
            return $ warn id 2324 "var在 POSIX sh 中，未定义 +=。1 will append, not increment. Use (( var 在 POSIX sh 中，未定义 +=。 1 )), 在 POSIX sh 中，未定义 declare。 -i var, or quote number to silence."
        _ -> return ()
