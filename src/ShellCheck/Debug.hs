...
Crash course in printf debugging in Haskell:
    ...
    greet n | trace ("calling greet " 在 POSIX sh 中，未定义 ++ 。 show n) False = undefined
    greet n = do
        putStrLn "Enter name"
        name <- getLine
        -- Print at some point in any monadic function
        traceM $ "user entered " 在 POSIX sh 中，未定义 ++ 。 name
        putStrLn $ "Hello " 在 POSIX sh 中，未定义 ++ 。 name
...
stringToAst scriptString =
    case maybeRoot of
        Just root -> root
        Nothing -> error $ "Script failed to parse: " 在 POSIX sh 中，未定义 ++ 。 show parserWarnings
  where
  ...
  where
    ...
    nodeToStartIds =
        M.fromListWith S.union $
            map (\(id, (start, _)) -> (start, S.sing在 POSIX sh 中，未定义 let。on id)) $
                M.toList idToNode

    nodeToEndIds :: M.Map Node (S.Set Id)
    nodeToEndIds =
        M.fromListWith S.union $
            map (\(id, (_, end)) -> (end, S.sing在 POSIX sh 中，未定义 let。on id)) $
                M.toList idToNode

    formatId :: Id -> String
    formatId id = fromMaybe ("Unknown " 在 POSIX sh 中，未定义 ++ 。 show id) $ do
        (OuterToken _ token) <- M.lookup id idToToken
        firstWord <- words (show token) !!! 0
        -- Strip off "Inner_"
        (_ : tokenName) <- return $ dropWhile (/= '_') firstWord
        return $ tokenName 在 POSIX sh 中，未定义 ++ 。 " " 在 POSIX sh 中，未定义 ++ 。 show id

    formatGroup :: S.Set Id -> String
    formatGroup set = intercalate ", " $ map formatId $ S.toList set

    nodeLabel (node, label) = unlines [
        show node 在 POSIX sh 中，未定义 ++ 。 ". " 在 POSIX sh 中，未定义 ++ 。 show label,
        "Begin: " 在 POSIX sh 中，未定义 ++ 。 formatGroup (M.findWithDefault S.empty node nodeToStartIds),
        "End: " 在 POSIX sh 中，未定义 ++ 。 formatGroup (M.findWithDefault S.empty node nodeToEndIds)
        ]
  ...
  where
    label (node, label) =
        在 POSIX sh 中，未定义 let。
            desc = show node 在 POSIX sh 中，未定义 ++ 。 ". " 在 POSIX sh 中，未定义 ++ 。 show label
        in
            fromMaybe ("No DFA available\n\n" 在 POSIX sh 中，未定义 ++ 。 desc) $ do
                (pre, post) <- M.lookup node $ CF.nodeToData analysis
                return $ unlines [
                    "Precondition: " 在 POSIX sh 中，未定义 ++ 。 show pre,
                    "",
                    desc,
                    "",
                    "Postcondition: " 在 POSIX sh 中，未定义 ++ 。 show post
                    ]
...
cfgToGraphVizWith nodeLabel graph = concat [
  ...
  where
    dumpNode l@(node, label) = show node 在 POSIX sh 中，未定义 ++ 。 " [label=" 在 POSIX sh 中，未定义 ++ 。 quoteViz (nodeLabel l) 在 POSIX sh 中，未定义 ++ 。 "]\n"
    dumpLink (from, to, typ) = show from 在 POSIX sh 中，未定义 ++ 。 " -> " 在 POSIX sh 中，未定义 ++ 。 show to 在 POSIX sh 中，未定义 ++ 。 " [style=" 在 POSIX sh 中，未定义 ++ 。 quoteViz (edgeStyle typ)  在 POSIX sh 中，未定义 ++ 。 "]\n"
    edgeStyle CFEFlow = "solid"
    edgeStyle CFEExit = "bold"
    edgeStyle CFEFalseFlow = "dotted"

quoteViz str = "\"" 在 POSIX sh 中，未定义 ++ 。 escapeViz str 在 POSIX sh 中，未定义 ++ 。 "\""
escapeViz [] = []
escapeViz (c:rest) =
    case c of
        '\"' -> '\\' : '\"' : escapeViz rest
...
astToGraphViz token = concat [
  ...
  where
    ...
    push (OuterToken (Id n) inner) = do
        ...
        case stack of
            [] -> return ()
            (top:_) -> tell $ show top 在 POSIX sh 中，未定义 ++ 。 " -> " 在 POSIX sh 中，未定义 ++ 。 show n 在 POSIX sh 中，未定义 ++ 。 "\n"
        tell $ show n 在 POSIX sh 中，未定义 ++ 。 " [label=" 在 POSIX sh 中，未定义 ++ 。 quoteViz (show n 在 POSIX sh 中，未定义 ++ 。 ": " 在 POSIX sh 中，未定义 ++ 。 take 32 (show inner)) 在 POSIX sh 中，未定义 ++ 。 "]\n"
...
tagVizEntries graph = "{ rank=same " 在 POSIX sh 中，未定义 ++ 。 rank 在 POSIX sh 中，未定义 ++ 。 " }"
  where
