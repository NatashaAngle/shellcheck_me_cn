...
instance ToJSON Replacement where
    toJSON replacement =
        在 POSIX sh 中，未定义 let。 start = repStartPos replacement
...
instance ToJSON PositionedComment where
  toJSON comment =
    在 POSIX sh 中，未定义 let。 start = pcStartPos comment
...
outputError file msg = hPutStrLn stderr $ file 在 POSIX sh 中，未定义 ++ 。 ": " 在 POSIX sh 中，未定义 ++ 。 msg
...
collectResult ref cr sys = mapM_ f groups
  where
    ...
    f group = do
        在 POSIX sh 中，未定义 let。 filename = sourceFile (NE.head group)
        result <- siReadFile sys (Just True) filename
        在 POSIX sh 中，未定义 let。 contents = either (const "") id result
        在 POSIX sh 中，未定义 let。 comments' = makeNonVirtual comments contents
        deepseq comments' $ modifyIORef ref (\x -> comments' 在 POSIX sh 中，未定义 ++ 。 x)
