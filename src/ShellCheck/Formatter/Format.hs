...
shouldOutputColor colorOption =
    case colorOption of
        ...
        ColorAuto -> do
            ...
            term <- lookupEnv "TERM"
            在 POSIX sh 中，未定义 let。 windows = "mingw" `isPrefixOf` os
            在 POSIX sh 中，未定义 let。 dumbTerm = term `elem` [Just "dumb", Just "", Nothing]
            在 POSIX sh 中，未定义 let。 isUsableTty = isTerminal && not windows && not dumbTerm
            return isUsableTty
