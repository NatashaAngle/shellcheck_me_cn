...
outputFile filename contents warnings = do
    在 POSIX sh 中，未定义 let。 comments = makeNonVirtual warnings contents
...
formatComment c = concat [
    ...
    attr "source" $ "ShellCheck.SC" 在 POSIX sh 中，未定义 ++ 。 show (codeNo c),
    "/>\n"
    ]
...
escape' c = if isOk c then [c] else "&#" 在 POSIX sh 中，未定义 ++ 。 show (ord c) 在 POSIX sh 中，未定义 ++ 。 ";"
