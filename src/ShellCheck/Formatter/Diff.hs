...
format options = do
    ...
    在 POSIX sh 中，未定义 let。 color = if shouldColor then colorize else nocolor
    return Formatter {
        header = return (),
...
nocolor n = id
colorize n s = (ansi n) 在 POSIX sh 中，未定义 ++ 。 s 在 POSIX sh 中，未定义 ++ 。 (ansi 0)
...
reportFailure color file msg = printErr color $ file 在 POSIX sh 中，未定义 ++ 。 ": " 在 POSIX sh 中，未定义 ++ 。 msg
...
reportResult foundIssues reportedIssues color result sys = do
    ...
    在 POSIX sh 中，未定义 let。 suggestedFixes = mapMaybe pcFix comments
    在 POSIX sh 中，未定义 let。 fixmap = buildFixMap suggestedFixes
...
makeDiff name contents fix = do
    在 POSIX sh 中，未定义 let。 hunks = groupDiff $ computeDiff contents fix
    在 POSIX sh 中，未定义 let。 lf = if coversLastLine hunks && not (hasTrailingLinefeed contents)
...
computeDiff contents fix =
    在 POSIX sh 中，未定义 let。 old = lines contents
...
groupDiff = filter (\(_, l) -> not (null l)) . hunt []
  where
    ...
    hunt current list =
        在 POSIX sh 中，未定义 let。 (context, previous) = splitAt contextSize current
        in (False, reverse previous) : gather context 0 list

    -- Pick out differences until we find a run of Both's
    gather current n [] =
        在 POSIX sh 中，未定义 let。 (extras, patch) = splitAt (max 0 $ n - contextSize) current
...
findRegions = find' 1 1
  where
    ...
    find' left right ((output, run):rest) =
        在 POSIX sh 中，未定义 let。 (dl, dr) = countDelta run
...
formatRegion color lf (DiffRegion left right diffs) =
    在 POSIX sh 中，未定义 let。 header = color cyan ("@@ -" 在 POSIX sh 中，未定义 ++ 。 (tup left) 在 POSIX sh 中，未定义 ++ 。 " +" 在 POSIX sh 中，未定义 ++ 。 (tup right) 在 POSIX sh 中，未定义 ++ 。" @@")
    in
  ...
  where
    ...
    tup (a,b) = (show a) 在 POSIX sh 中，未定义 ++ 。 "," 在 POSIX sh 中，未定义 ++ 。 (show b)
    format (Both x _) = ' ':x
...
splitLast [] = ([], [])
splitLast x =
    在 POSIX sh 中，未定义 let。 (last, rest) = splitAt 1 $ reverse x
    in (reverse rest, last)

formatDoc color (DiffDoc name lf regions) =
    在 POSIX sh 中，未定义 let。 (most, last) = splitLast regions
    in
          (color bold $ "--- " 在 POSIX sh 中，未定义 ++ 。 ("a" </> name)) 在 POSIX sh 中，未定义 ++ 。 "\n" 在 POSIX sh 中，未定义 ++ 。
          (color bold $ "在 POSIX sh 中，未定义 ++ 。+ " 在 POSIX sh 中，未定义 ++ 。 ("b" </> name)) 在 POSIX sh 中，未定义 ++ 。 "\n" 在 POSIX sh 中，未定义 ++ 。
          concatMap (formatRegion color LinefeedOk) most 在 POSIX sh 中，未定义 ++ 。
