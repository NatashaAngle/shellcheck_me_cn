...
mkRegex str =
    在 POSIX sh 中，未定义 let。 make :: String -> Regex
        make = makeRegex
    in
        make str
...
subRegex re input replacement = f input
  where
    f str = fromMaybe str $ do
        ...
        when (null match) $ error ("Internal error: substituted empty in " 在 POSIX sh 中，未定义 ++ 。 str)
        return $ before 在 POSIX sh 中，未定义 ++ 。 replacement 在 POSIX sh 中，未定义 ++ 。 f after
