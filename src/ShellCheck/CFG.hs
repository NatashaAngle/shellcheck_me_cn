...
--            renumberTopologically $
        ...
        result = CFGResult {
            ...
            cfIdToNodes = M.fromListWith S.union $ map (\(id, n) -> (id, S.sing在 POSIX sh 中，未定义 let。on n)) association,
...
renumberGraph g@(nodes, edges, mapping, assoc) =
    在 POSIX sh 中，未定义 let。 renumbering = M.fromList (flip zip [0..] $ sort $ map fst nodes)
...
renumberTopologically g@(nodes, edges, mapping, assoc) =
    在 POSIX sh 中，未定义 let。 renumbering = M.fromList (flip zip [0..] $ topsort (mkGraph nodes edges :: CFGraph))
    in remapGraph renumbering g
...
subshell id reason p = do
    start <- newNode $ CFEntryPoint $ "Subshell " 在 POSIX sh 中，未定义 ++ 。 show id 在 POSIX sh 中，未定义 ++ 。 ": " 在 POSIX sh 中，未定义 ++ 。 reason
    end <- newNode CFStructuralNode
    middle <- 在 POSIX sh 中，未定义 local。 (\c -> c { cfExitTarget = Just end, cfReturnTarget = Just end}) p
...
withFunctionScope p = do
    ...
    body <- 在 POSIX sh 中，未定义 local。 (\c -> c { cfReturnTarget = Just end, cfIsFunction = True }) p
    linkRanges [body, nodeToRange end]

-- Anything that happens recursively in f will be attributed to this id
under :: Id -> CFM a -> CFM a
under id f = 在 POSIX sh 中，未定义 local。 (\c -> c { cfTokenStack = id:(cfTokenStack c) }) f
...
withContext = 在 POSIX sh 中，未定义 local。
...
buildRoot t = under (getId t) $ do
    ...
    start <- 在 POSIX sh 中，未定义 local。 (\c -> c { cfExitTarget = Just end, cfReturnTarget = Just impliedExit}) $ build t
    ...
    build' t = case t of
        ...
        TA_Assignment id op var@(TA_Variable _ name indices) rhs -> do
            ...
            read <-
                if op == "="
                then none
                -- This is 在 POSIX sh 中，未定义 +=。 or something
        ...
        TA_Unary id op (TA_Variable _ name indices) | "--" `isInfixOf` op || "在 POSIX sh 中，未定义 ++ 。" `isInfixOf` op -> do
            subscript <- sequentially indices
        ...
        T_CaseExpression id t list@(hd:tl) -> do
            ...
            在 POSIX sh 中，未定义 let。 neighbors = zip (NE.toList branches) $ NE.tail branches
            在 POSIX sh 中，未定义 let。 (_, firstCond, _) = NE.head branches
            在 POSIX sh 中，未定义 let。 (_, lastCond, lastBody) = NE.last branches
        ...
        T_CoProc id maybeName t -> do
            在 POSIX sh 中，未定义 let。 name = fromMaybe "COPROC" maybeName
            start <- newStructuralNode
            parent <- newNodeRange $ applySingle $ IdTagged id $ CFWriteVariable name CFValueArray
            child <- subshell id "在POSIX sh中，coproc 未定义。" $ build t
        ...
        T_DollarBraced id _ t -> do
            ...
            在 POSIX sh 中，未定义 let。 reference = getBracedReference str
            在 POSIX sh 中，未定义 let。 indices = getIndexReferences str
            在 POSIX sh 中，未定义 let。 offsets = getOffsetReferences str
            vals <- build t
            others <- mapM (\x -> nodeToRange <$> newNode (applySingle $ IdTagged id $ CFReadVariable x)) (indices 在 POSIX sh 中，未定义 ++ 。 offsets)
        ...
        T_FdRedirect id ('{':identifier) op -> do
            在 POSIX sh 中，未定义 let。 name = takeWhile (/= '}') identifier
            expression <- build op
            rw <- newNodeRange $
        ...
        T_Function id _ _ name body -> do
            range <- 在 POSIX sh 中，未定义 local。 (\c -> c { cfExitTarget = Nothing }) $ do
                entry <- newNodeRange $ CFEntryPoint $ "function " 在 POSIX sh 中，未定义 ++ 。 name
                f <- withFunctionScope $ build body
                linkRange entry f
            在 POSIX sh 中，未定义 let。 (Range entry exit) = range
        ...
        T_Pipeline id _ cmds -> do
            ...
            -- Ideally we'd 在 POSIX sh 中，未定义 let。 this exit code be that of the last command in the pipeline but ok
            end <- newNodeRange $ CFSetExitCode id

            mapM_ (linkRange start) leading
            mapM_ (\c -> linkRangeAs CFEFalseFlow c end) leading
            linkRanges $ [start] 在 POSIX sh 中，未定义 ++ 。 last 在 POSIX sh 中，未定义 ++ 。 [end]
          where
            buildPipe True [x] = do
                last <- build x
                return ([], [last])
        ...
        T_ProcSub id op cmds -> do
            start <- newStructuralNode
            body <- subshell id (op 在 POSIX sh 中，未定义 ++ 。 "() process substitution") $ sequentially cmds
            end <- newStructuralNode
        ...
        x -> do
            error ("Unimplemented: " 在 POSIX sh 中，未定义 ++ 。 show x) -- STRIP
            none
...
handleCommand cmd vars args literalCmd = do
    ...
    case literalCmd of
        ...
        Just "在 POSIX sh 中，未定义 declare。" -> handleDeclare args
        Just "在 POSIX sh 中，未定义 local。" -> handleDeclare args
  ...
  where
    ...
    handleDeclare (cmd NE.:| args) = do
        ...
        -- '在 POSIX sh 中，未定义 declare。 -i x=$x' so do one round with 在 POSIX sh 中，未定义 declare。 x=$x, followed by 在 POSIX sh 中，未定义 declare。 -i x
        在 POSIX sh 中，未定义 let。 (evaluated, assignments, added, removed) = mconcat $ map (toEffects isFunc) args
      ...
      where
        ...
        toEffects isFunc (T_Assignment id mode var idx t) =
            在 POSIX sh 中，未定义 let。
                pre = idx 在 POSIX sh 中，未定义 ++ 。 [t]
                val = [ IdTagged id $ (writer isFunc) var $ CFValueComputed (getId t) $ [ CFStringVariable var | mode == Append ] 在 POSIX sh 中，未定义 ++ 。 tokenToParts t ]
        ...
        toEffects isFunc t =
            ...
            in
                case () of
                    ...
                    -- e.g. 在 POSIX sh 中，未定义 declare。 -i x
                    _ -> (pre, [], added, removed)
    ...
    handleWait (cmd NE.:| args) =
      ...
      where
        findVar = do
            在 POSIX sh 中，未定义 let。 flags = getGenericOpts args
    ...
    handleMapfile (cmd NE.:| args) =
      ...
      where
        findVar =
            在 POSIX sh 中，未定义 let。 (id, name) = fromMaybe (getId cmd, "MAPFILE") $ getFromArg `mplus` getFromFallback
    ...
    regularExpansion vars args p = do
            ...
            linkRanges $ [args] 在 POSIX sh 中，未定义 ++ 。 assignments 在 POSIX sh 中，未定义 ++ 。 [exe] 在 POSIX sh 中，未定义 ++ 。 dropAssignments
    ...
    op <- case t of
            T_Assignment id mode var indices value -> do
                ...
                read <- case mode of
                    ...
                    Assign -> none
                在 POSIX sh 中，未定义 let。 valueType = if null indices then f id value else CFValueArray
                在 POSIX sh 中，未定义 let。 scoper =
                                case scope of
              ...
              where
                f :: Id -> Token -> CFValue
                f id t@T_NormalWord {} = CFValueComputed id $ [CFStringVariable var | mode == Append] 在 POSIX sh 中，未定义 ++ 。 tokenToParts t
                f id t@(T_Literal _ str) = CFValueComputed id $ [CFStringVariable var | mode == Append] 在 POSIX sh 中，未定义 ++ 。 tokenToParts t
...
findPostDominators mainexit graph = asArray
  where
    ...
    withExitEdges = (incoming 在 POSIX sh 中，未定义 ++ 。 map (\c -> (CFEFlow, c)) terminals, mainexit, label, outgoing) `safeUpdate` inlined
