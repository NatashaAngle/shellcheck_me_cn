...
checkScript sys spec = do
  ...
  where
    checkScript contents = do
        ...
        在 POSIX sh 中，未定义 let。 parseMessages = prComments result
        在 POSIX sh 中，未定义 let。 tokenPositions = prTokenPositions result
        在 POSIX sh 中，未定义 let。 analysisSpec root =
                as {
                    asScript = root,
                    ...
                    asOptionalChecks = getEnableDirectives root 在 POSIX sh 中，未定义 ++ 。 csOptionalChecks spec
                } where as = newAnalysisSpec root
        在 POSIX sh 中，未定义 let。 analysisMessages =
                maybe []
                    (arComments . analyzeScript . analysisSpec)
                        $ prRoot result
        在 POSIX sh 中，未定义 let。 translator = tokenToPosition tokenPositions
        return . nub . sortMessages . filter shouldInclude $
            (parseMessages 在 POSIX sh 中，未定义 ++ 。 map translator analysisMessages)
    ...
    order pc =
        在 POSIX sh 中，未定义 let。 pos = pcStartPos pc
