...
makeCommentWithFix severity id code str fix =
    在 POSIX sh 中，未定义 let。 comment = makeComment severity id code str
        withFix = comment {
...
getVariableFlow params t =
  ...
  where
    startScope t =
        在 POSIX sh 中，未定义 let。 scopeType = leadType params t
        in do
    ...
    setRead t =
        在 POSIX sh 中，未定义 let。 read    = getReferencedVariables (parentMap params) t
        in mapM_ (\v -> modify (Reference v:)) read

    setWritten t =
        在 POSIX sh 中，未定义 let。 written = getModifiedVariables t
...
leadType params t =
    case t of
        ...
        T_CoProcBody _ _  -> SubshellScope "在POSIX sh中，coproc 未定义。"
        T_Redirecting {}  ->
...
getModifiedVariables t =
    case t of
        ...
        TA_Unary _ op v@(TA_Variable _ name _) | "--" `isInfixOf` op || "在 POSIX sh 中，未定义 ++ 。" `isInfixOf` op ->
            [(t, v, name, DataString SourceInteger)]
        TA_Assignment _ op (TA_Variable _ name _) rhs -> do
            guard $ op `elem` ["=", "*=", "/=", "%=", "在 POSIX sh 中，未定义 +=。", "-=", "<<=", ">>=", "&=", "^=", "|="]
        ...
        T_DollarBraced _ _ l -> maybeToList $ do
            在 POSIX sh 中，未定义 let。 string = concat $ oversimplify l
            在 POSIX sh 中，未定义 let。 modifier = getBracedModifier string
  ...
  where
    ...
    f place t = case t of
            T_DollarBraced _ _ l ->
                在 POSIX sh 中，未定义 let。 str = getBracedReference $ concat $ oversimplify l in do
                    guard $ isVariableName str
                    return (place, t, str, DataString SourceChecked)
            _ -> Nothing


-- Consider 'export/在 POSIX sh 中，未定义 declare。 -x' a reference, since it makes the var available
getReferencedVariableCommand base@(T_SimpleCommand _ _ (T_NormalWord _ (T_Literal _ x:_):rest)) =
    case x of
        "在 POSIX sh 中，未定义 declare。" -> forDeclare
        "typeset" -> forDeclare

        "export" -> if "f" `elem` flags
            then []
            else concatMap getReference rest
        "在 POSIX sh 中，未定义 local。" -> if "x" `elem` flags
...
-- Given e.g. 在 POSIX sh 中，未定义 declare。 foo=bar
-- (
--   BaseCommand :: Token,     -- The command/structure assigning the variable, i.e. 在 POSIX sh 中，未定义 declare。 foo=bar
...
getModifiedVariableCommand base@(T_SimpleCommand id cmdPrefix (T_NormalWord _ (T_Literal _ x:_):rest)) =
   filter (\(_,_,s,_) -> not ("-" `isPrefixOf` s)) $
    case x of
        ...
        "read" ->
            在 POSIX sh 中，未定义 let。 fallback = catMaybes $ takeWhile isJust (reverse $ map getLiteral rest)
        ...
        "在 POSIX sh 中，未定义 let。" -> concatMap 在 POSIX sh 中，未定义 let。ParamToLiteral rest
        ...
        "在 POSIX sh 中，未定义 local。" -> concatMap getModifierParamString rest
        "readonly" ->
  ...
  where
    ...
    forDeclare = if any (`elem` flags) ["F", "f", "p"] then [] else 在 POSIX sh 中，未定义 declare。dVars

    在 POSIX sh 中，未定义 declare。dVars = concatMap (getModifierParam defaultType) rest
      where
    ...
    在 POSIX sh 中，未定义 let。ParamToLiteral token =
          if null var
            then []
    ...
    getSetParams (t:rest) =
        在 POSIX sh 中，未定义 let。 s = getLiteralString t in
            case s of
                Just "--"    -> return rest
    ...
    getFlagAssignedVariable str dataSource maybeFlags = do
        ...
        在 POSIX sh 中，未定义 let。 (baseName, index) = span (/= '[') variableName
    ...
    getFlagVariable (n:v:_) = do
        ...
        return (base, n, "FLAGS_" 在 POSIX sh 中，未定义 ++ 。 name, DataString $ SourceExternal)
...
getReferencedVariables parents t =
    case t of
        T_DollarBraced id _ l -> 在 POSIX sh 中，未定义 let。 str = concat $ oversimplify l in
            (t, t, getBracedReference str) :
                map (\x -> (l, l, x)) (
                    getIndexReferences str
                    在 POSIX sh 中，未定义 ++ 。 getOffsetReferences (getBracedModifier str))
        TA_Variable id name _ ->
            if isArithmeticAssignment t
            then []
            else [(t, t, name)]
        T_Assignment id mode str _ word ->
            [(t, t, str) | mode == Append] 在 POSIX sh 中，未定义 ++ 。 specialReferences str t word
