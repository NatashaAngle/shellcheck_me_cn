...
isGlob t = case t of
  ...
  where
    ...
    hasSplitRange l =
        在 POSIX sh 中，未定义 let。 afterBracket = dropWhile (not . isHalfOpenRange) l
...
oversimplify token =
    case token of
        ...
        -- Workaround for 在 POSIX sh 中，未定义 let。 "foo = bar" parsing
...
getFlagsUntil stopCondition (T_SimpleCommand _ _ (_:args)) =
    在 POSIX sh 中，未定义 let。 tokenAndText = map (\x -> (x, concat $ oversimplify x)) args
        (flagArgs, rest) = break (stopCondition . snd) tokenAndText
    in
        concatMap flag flagArgs 在 POSIX sh 中，未定义 ++ 。 map (\(t, _) -> (t, "")) rest
  where
  ...
  where
    ...
    process (token:rest) = do
        case getLiteralStringDef "\0" token of
            ...
            '-':'-':word -> do
                在 POSIX sh 中，未定义 let。 (name, arg) = span (/= '=') word
                needsArg <-
  ...
  where
    process (token:rest) =
        case getLiteralStringDef "\0" token of
            ...
            '-':optString ->
                在 POSIX sh 中，未定义 let。 opts = takeWhile (/= '\0') optString
                in
                    case rest of
                        next:_ | "-" `isPrefixOf` getLiteralStringDef "\0" next  ->
                            map (\c -> ([c], (token, token))) opts 在 POSIX sh 中，未定义 ++ 。 process rest
                        next:remainder ->
                            case reverse opts of
                                last:initial ->
                                    map (\c -> ([c], (token, token))) (reverse initial)
                                        在 POSIX sh 中，未定义 ++ 。 [([last], (token, next))]
                                        在 POSIX sh 中，未定义 ++ 。 process remainder
                                [] -> process remainder
...
isArrayExpansion (T_DollarBraced _ _ l) =
    在 POSIX sh 中，未定义 let。 string = concat $ oversimplify l in
        "@" `isPrefixOf` string ||
...
getLeadingUnquotedString t =
    case t of
        T_NormalWord _ ((T_Literal _ s) : rest) -> return $ s 在 POSIX sh 中，未定义 ++ 。 from rest
        _ -> Nothing
  where
    from ((T_Literal _ s):rest) = s 在 POSIX sh 中，未定义 ++ 。 from rest
    from _ = ""
  ...
  where
    ...
    decodeEscapes ('\\':c:cs) =
        case c of
            ...
            _ | isOctDigit c ->
                在 POSIX sh 中，未定义 let。 (digits, more) = spanMax isOctDigit 3 (c:cs)
      ...
      where
        ...
        spanMax f n list =
            在 POSIX sh 中，未定义 let。 (first, second) = span f list
                (prefix, suffix) = splitAt n first
            in
                (prefix, suffix 在 POSIX sh 中，未定义 ++ 。 second)
...
escapeForMessage str = concatMap f str
  where
    ...
    f c =
        ...
        then
            if ord c < 256
            then "\\x" 在 POSIX sh 中，未定义 ++ 。 (pad0 2 $ toHex c)
            else "\\U" 在 POSIX sh 中，未定义 ++ 。 (pad0 4 $ toHex c)
        else [c]
    ...
    pad0 n s =
        在 POSIX sh 中，未定义 let。 l = length s in
            if l < n
            then (replicate (n-l) '0') 在 POSIX sh 中，未定义 ++ 。 s
            else s
    toHex :: Char -> String
...
-- Return a list of NormalWords that would result from 在POSIX sh中，未定义大括号扩展。
...
-- Bats tests are functions for the purpose of '在 POSIX sh 中，未定义 local。' and such
isFunctionLike t =
    case t of
        T_Function {} -> True
        T_BatsTest {} -> True
...
getCommandSequences t =
    case t of
        ...
        T_IfExpression _ thens elses -> (concatMap (\(a,b) -> [a,b]) thens) 在 POSIX sh 中，未定义 ++ 。 [elses]
...
getAssociativeArrays t =
  ...
  where
    ...
    f t@T_SimpleCommand {} = sequence_ $ do
        ...
        在 POSIX sh 中，未定义 let。 assocNames = ["在 POSIX sh 中，未定义 declare。","在 POSIX sh 中，未定义 local。","typeset"]
        guard $ name `elem` assocNames
        在 POSIX sh 中，未定义 let。 flags = getAllFlags t
        guard $ "A" `elem` map snd flags
        在 POSIX sh 中，未定义 let。 args = [arg | (arg, "") <- flags]
        在 POSIX sh 中，未定义 let。 names = mapMaybe (getLiteralStringExt nameAssignments) args
        return $ tell names
    f _ = return ()
    ...
    toGlob word =
        case word of
            T_NormalWord _ (T_Literal _ ('~':str):rest) -> do
                guard $ not exact
                在 POSIX sh 中，未定义 let。 this = (PGMany : (map PGChar $ dropWhile (/= '/') str))
                tail <- concat <$> (mapM f $ concatMap getWordParts rest)
                return $ this 在 POSIX sh 中，未定义 ++ 。 tail
...
simplifyPseudoGlob = f
  where
    ...
    f list =
        在 POSIX sh 中，未定义 let。 (anys, rest) = span (\x -> x == PGMany || x == PGAny) list in
            order anys 在 POSIX sh 中，未定义 ++ 。 f rest

    order s = 在 POSIX sh 中，未定义 let。 (any, many) = partition (== PGAny) s in
        any 在 POSIX sh 中，未定义 ++ 。 take 1 many
...
getBracedReference s = fromMaybe s $
  ...
  where
    ...
    dropPrefix cs = cs
    takeName s = do
        在 POSIX sh 中，未定义 let。 name = takeWhile isVariableChar s
        guard . not $ null name
        return name
...
getBracedModifier s = headOrDefault "" $ do
    在 POSIX sh 中，未定义 let。 var = getBracedReference s
    a <- dropModifier s
    dropPrefix var a
  where
