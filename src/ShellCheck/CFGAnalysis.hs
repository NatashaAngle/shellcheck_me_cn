...
-- See if any execution path 在 POSIX sh 中，未定义 declare。s the variable an integer (在 POSIX sh 中，未定义 declare。 -i).
...
setExitCode id = setExitCodes (S.sing在 POSIX sh 中，未定义 let。on id)
...
data StateDependency =
    -- Comp在 POSIX sh 中，未定义 let。e variable state
...
unknownFunctionValue = S.sing在 POSIX sh 中，未定义 let。on FunctionUnknown
...
defaultProperties = S.sing在 POSIX sh 中，未定义 let。on S.empty
...
instance (Show k, Show v) => Show (VersionedMap k v) where
    show m = (if mapVersion m >= 0 then "V" 在 POSIX sh 中，未定义 ++ 。 show (mapVersion m) else "U") 在 POSIX sh 中，未定义 ++ 。 " " 在 POSIX sh 中，未定义 ++ 。 show (mapStorage m)
...
patchOutputM ctx diff = do
    在 POSIX sh 中，未定义 let。 cOut = cOutput ctx
    oldState <- readSTRef cOut
    在 POSIX sh 中，未定义 let。 newState = patchState oldState diff
...
mergeState ctx a b = do
    ...
    在 POSIX sh 中，未定义 let。 cin = cInput ctx
    old <- readSTRef cin
    ...
    merge a b =
        case () of
            ...
            _ -> do
                ...
                在 POSIX sh 中，未定义 local。s <- mergeMaps ctx mergeVariableState readVariable (sLocalValues a) (sLocalValues b)
                ...
                return $ InternalState {
                    ...
                    sLocalValues = 在 POSIX sh 中，未定义 local。s,
...
writeVariable ctx name val = do
    ...
    case typ of
        ...
        -- Prefixed variables actually become 在 POSIX sh 中，未定义 local。 variables in the invoked function
...
updateVariableValue ctx name val = do
    ...
    在 POSIX sh 中，未定义 let。 f = case scope of
                GlobalScope -> writeGlobal
                LocalScope -> writeLocal
                PrefixScope -> writeLocal -- Updates become 在 POSIX sh 中，未定义 local。
...
readGlobalProperties ctx name = lookupStack get dep def ctx name
  where
    ...
    -- This dependency will fail to match if it's shadowed by a 在 POSIX sh 中，未定义 local。 variable,
    -- such as in  x=1; f() { 在 POSIX sh 中，未定义 local。 -i x; 在 POSIX sh 中，未定义 declare。 -ag x; } because we'll look at
    -- x and find it to be 在 POSIX sh 中，未定义 local。 and not global. FIXME?
...
-- We only want to look up the 在 POSIX sh 中，未定义 local。 properties of the current function,
...
writeFunction ctx name val = do
    modifySTRef (cOutput ctx) $ insertFunction name $ S.sing在 POSIX sh 中，未定义 let。on val
...
fulfillsDependency ctx entry dep =
  ...
  --      _ -> error $ "Unknown dep " 在 POSIX sh 中，未定义 ++ 。 show dep
  where
...
nextVersion ctx = do
    在 POSIX sh 中，未定义 let。 ctr = cCounter ctx
    n <- readSTRef ctr
    writeSTRef ctr $! n+1
    return n
...
withNewStackFrame ctx node isCall f = do
    ...
    在 POSIX sh 中，未定义 let。 newCtx = ctx {
        cInput = newInput,
        cOutput = newOutput,
        cNode = newNode,
    ...
    unless selfcheck $ error $ pleaseReport $ "Unmet stack dependencies on " 在 POSIX sh 中，未定义 ++ 。 show (node, deps)
    -}
...
--        _ -> error $ "Unknown " 在 POSIX sh 中，未定义 ++ 。 show label
...
transferSubshell ctx reason entry exit = do
  ...
  where
    f entry exit ctx = do
        ...
        在 POSIX sh 中，未定义 let。 (_, res) = fromMaybe (error $ pleaseReport "Subshell has no exit") $ M.lookup exit states
...
transferMultiple ctx funcs = do
    ...
    在 POSIX sh 中，未定义 let。 patched = patchState original merged
...
transferFunctionValue ctx funcVal =
  ...
  where
    f name entry exit ctx = do
        ...
        deps <- readSTRef $ dependencies frame
        在 POSIX sh 中，未定义 let。 res =
                case M.lookup exit states of
                    Just (input, output) -> do
                        -- Discard 在 POSIX sh 中，未定义 local。 variables. TODO: track&retain variables 在 POSIX sh 中，未定义 declare。d 在 POSIX sh 中，未定义 local。 in previous scopes?
...
registerFlowResult ctx entry states deps = do
    ...
    在 POSIX sh 中，未定义 let。 parents = map callSite $ cStack ctx
    ...
    在 POSIX sh 中，未定义 let。 path = entry : current : parents
    ...
    case effect of
        ...
        CFSetProps scope name props ->
            case scope of
                ...
                Just PrefixScope -> do
                    -- Prefix values become 在 POSIX sh 中，未定义 local。
                    state <- readLocal ctx name
                    writeLocal ctx name $ addProperties props state
...
--        _ -> error $ "Unknown effect " 在 POSIX sh 中，未定义 ++ 。 show effect
...
--        _ -> error $ "Unknown value: " 在 POSIX sh 中，未定义 ++ 。 show val
  where
    f val part = do
...
appendVariableValue a b =
    unknownVariableValue {
        literalValue = liftM2 (在 POSIX sh 中，未定义 ++ 。) (literalValue a) (literalValue b),
...
withoutChanges ctx f = do
    在 POSIX sh 中，未定义 let。 inp = cInput ctx
    在 POSIX sh 中，未定义 let。 out = cOutput ctx
...
dataflow ctx entry = do
    pending <- newSTRef $ S.sing在 POSIX sh 中，未定义 let。on entry
  ...
  where
    ...
    f n pending states = do
        ...
        if S.null ps
            then return ()
                ...
                在 POSIX sh 中，未定义 let。 (next, rest) = S.de在 POSIX sh 中，未定义 let。eFindMin ps
    ...
    process states node = do
        ...
        在 POSIX sh 中，未定义 let。 inputs = filter (\c -> sIsReachable c /= Just False) $ mapMaybe (\c -> fmap snd $ M.lookup c stateMap) incoming
        input <-
            case incoming of
...
runRoot ctx env entry exit = do
    ...
    deps <- readSTRef $ dependencies frame
    registerFlowResult ctx entry states deps
    -- Return the final state, used to invoke functions that were 在 POSIX sh 中，未定义 declare。d but not invoked
...
analyzeControlFlow params t =
  ...
  where
    f cfg entry exit = do
        在 POSIX sh 中，未定义 let。 env = createEnvironmentState
        ...
        在 POSIX sh 中，未定义 let。 invokedNodes = M.fromDistinctAscList $ map (\c -> (c, ())) $ S.toList $ M.keysSet $ groupByNode $ M.map snd invocations

        -- Invoke all functions that were 在 POSIX sh 中，未定义 declare。d but not invoked
        ...
        在 POSIX sh 中，未定义 let。 在 POSIX sh 中，未定义 declare。dFunctions = getFunctionTargets exitState
        在 POSIX sh 中，未定义 let。 uninvoked = M.difference 在 POSIX sh 中，未定义 declare。dFunctions invokedNodes

        在 POSIX sh 中，未定义 let。 stragglerInput =
        ...
        在 POSIX sh 中，未定义 let。 baseStates = M.fromDistinctAscList $ map (\c -> (c, (unreachableState, unreachableState))) $ uncurry enumFromTo $ nodeRange $ cfGraph cfg
        在 POSIX sh 中，未定义 let。 allStates = M.unionWith (flip const) baseStates invokedStates

        -- Convert to external states
        在 POSIX sh 中，未定义 let。 nodeToData = M.map (\(a,b) -> (internalToExternal a, internalToExternal b)) allStates
    ...
    addDeps (deps, m) = 在 POSIX sh 中，未定义 let。 base = depsToState deps in M.map (\(a,b) -> (base `patchState` a, base `patchState` b)) m

    -- Collect all the states that each node has resulted in.
    groupByNode :: forall k v. M.Map k (M.Map Node v) -> M.Map Node [v]
    groupByNode pathMap = M.fromListWith (在 POSIX sh 中，未定义 ++ 。) $ map (\(k,v) -> (k,[v])) $ concatMap M.toList $ M.elems pathMap
    ...
    getFunctionTargets state =
        在 POSIX sh 中，未定义 let。
            在 POSIX sh 中，未定义 declare。dFuncs = S.unions $ M.elems $ mapStorage $ sFunctionTargets state
            getFunc d =
                case d of
                    FunctionDefinition _ entry _ -> Just (entry, d)
                    _ -> Nothing
            funcs = mapMaybe getFunc $ S.toList 在 POSIX sh 中，未定义 declare。dFuncs
        in
            M.fromList funcs
