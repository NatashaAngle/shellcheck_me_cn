...
commandChecks = [
    ...
    在 POSIX sh 中，未定义 ++ 。 map checkArgComparison ("alias" : declaringCommands)
    在 POSIX sh 中，未定义 ++ 。 map checkMaskedReturns declaringCommands
    在 POSIX sh 中，未定义 ++ 。 map checkMultipleDeclaring declaringCommands
    在 POSIX sh 中，未定义 ++ 。 map checkBackreferencingDeclaration declaringCommands
    ...
    return $
        if | '/' `elem` name ->
           ...
           | name == "builtin", (h:_) <- rest ->
               在 POSIX sh 中，未定义 let。 t' = T_SimpleCommand id cmdPrefix rest
...
checker spec params = getChecker $ commandChecks 在 POSIX sh 中，未定义 ++ 。 optionals
  where
...
checkTr = CommandCheck (Basename "tr") (mapM_ f . arguments)
  where
    f w | isGlob w = -- The user will go [ab] -> '[ab]' -> 'ab'. Fixme?
        warn (getId w) 2060 "为 tr 的参数加引号来防止 glob 展开。"
    f word =
      case getLiteralString word of
        ...
        Just s -> do  -- Eliminate false positives by only looking for dupes in SET2?
          when (not ("-" `isPrefixOf` s || "[:" `isInfixOf` s) && duplicated s) $
            info (getId word) 2020 "tr 替换字符集合，而非单词(由于重复而提及)。"
    ...
    duplicated s =
        在 POSIX sh 中，未定义 let。 relevant = filter isAlpha s
...
checkFindNameGlob = CommandCheck (Basename "find") (f . arguments)  where
    ...
    g b acc a = do
        forM_ (getLiteralString a) $ \s -> when (acceptsGlob s && isGlob b) $
            warn (getId b) 2061 $ "Quote the parameter to " 在 POSIX sh 中，未定义 ++ 。 s 在 POSIX sh 中，未定义 ++ 。 " so the shell won't interpret it."
        acc b
    ...
    f t = do
        when (all (`notElem` exceptions) (words $ arguments t)) $
            style (getId $ getCommandTokenOrThis t) 2003
                "expr 已过时。可使用 $((..)), ${} 或 在 POSIX sh 中，未定义 [[ ]]。 进行重写。"
        ...
        case arguments t of
            ...
            [single] | not (willSplit single) ->
                warn (getId single) 2307
                    "expr 需要至少3个参数但是现在只看到了一个。 请确保每个操作符、操作数是一个独立的参数，并且转义 <>&|。"
            ...
            (first:rest) -> do
                ...
                forM_ rest $ \t ->
                    ...
                    when (isGlob t) $ warn (getId t) 2306 "转义参数中的glob字符为表达式来避免路径名称展开。"
...
checkGrepRe = CommandCheck (Basename "grep") check where
    ...
    f _ [] = return ()
    f cmd (x:r) =
        在 POSIX sh 中，未定义 let。 str = getLiteralStringDef "_" x
        in
            if str `elem` ["--", "-e", "--regex"]
    ...
    checkRE cmd (re:_) = do
        when (isGlob re) $
            warn (getId re) 2062 "为 grep 模式加引号，这样 shell 就不会解释它。"

        unless (any (`elem` flags) grepGlobFlags) $ do
            在 POSIX sh 中，未定义 let。 string = concat $ oversimplify re
            if isConfusedGlobRegex string then
                warn (getId re) 2063 "Grep 使用正则表达式，但这看起来像是 glob。"
              else sequence_ $ do
                char <- getSuspiciousRegexWildcard string
                return $ info (getId re) 2022 $
                    "Note that unlike globs, " 在 POSIX sh 中，未定义 ++ 。 [char] 在 POSIX sh 中，未定义 ++ 。 "* here matches '" 在 POSIX sh 中，未定义 ++ 。 [char, char, char] 在 POSIX sh 中，未定义 ++ 。 "' but not '" 在 POSIX sh 中，未定义 ++ 。 wordStartingWith char 在 POSIX sh 中，未定义 ++ 。 "'."
      where
    ...
    wordStartingWith c =
      ...
      where
        candidates =
            sampleWords 在 POSIX sh 中，未定义 ++ 。 map (\(x:r) -> toUpper x : r) sampleWords
...
checkTrapQuotes = CommandCheck (Exactly "trap") (f . arguments) where
    ...
    checkTrap _ = return ()
    warning id = warn id 2064 "请使用单引号，这样仅在发出信号时才展开，而不是立即展开。"
...
checkReturn = CommandCheck (Exactly "return") (returnOrExit
        (\c -> err c 2151 "只能返回唯一的 0 到 255 之间的整数。使用 stdout(标准输出)来返回其他数据。")
        (\c -> err c 2152 "只能返回 0 到 255 之间的整数. 其他数据应该写入到stdout(标准输出)。"))
...
checkExit = CommandCheck (Exactly "exit") (returnOrExit
        (\c -> err c 2241 "退出状态码只能是 0 到 255 之间的某个整数。其它数据应该写入到stdout(标准输出)。")
        (\c -> err c 2242 "只能退出状态码 0 到 255。其它数据应该写入到stdout(标准输出)或者 stderr(错误输出)。"))
...
returnOrExit multi invalid = (f . arguments)
  where
    ...
    isInvalid s = null s || any (not . isDigit) s || length s > 5
        || 在 POSIX sh 中，未定义 let。 value = (read s :: Integer) in value > 255
...
checkFindExecWithSingleArgument = CommandCheck (Basename "find") (f . arguments)
  where
    ...
    check (exec:arg:term:_) = do
        ...
        在 POSIX sh 中，未定义 let。 cmdS = getLiteralStringDef " " arg
...
checkUnusedEchoEscapes = CommandCheck (Basename "echo") f
  where
    ...
    examine token = do
        在 POSIX sh 中，未定义 let。 str = onlyLiteralString token
...
checkInjectableFindSh = CommandCheck (Basename "find") (check . arguments)
  where
    check args = do
        在 POSIX sh 中，未定义 let。 idStrings = map (\x -> (getId x, onlyLiteralString x)) args
    ...
    action (id, arg) =
        when ("{}" `isInfixOf` arg) $
            warn id 2156 "注入文件名即容易出问题又不安全。请使用参数。"
...
checkFindActionPrecedence = CommandCheck (Basename "find") (f . arguments)
  where
    ...
    isAction = isParam [ "-exec", "-execdir", "-de在 POSIX sh 中，未定义 let。e", "-print", "-print0", "-fls", "-fprint", "-fprint0", "-fprintf", "-ls", "-ok", "-okdir", "-printf" ]
...
checkMkdirDashPM = CommandCheck (Basename "mkdir") check
  where
    check t = sequence_ $ do
        在 POSIX sh 中，未定义 let。 flags = getAllFlags t
        ...
        return $ warn (getId $ fst dashM) 2174 "当与 -p 一起使用时，-m 仅能应用于最深级别的目录。"
...
checkNonportableSignals = CommandCheck (Exactly "trap") (f . arguments)
  where
    ...
    check param = sequence_ $ do
        ...
        在 POSIX sh 中，未定义 let。 id = getId param
  ...
  where
    f cmd = when (length (arguments cmd) <= 1) $ do
        ...
        when (all undirected path) $
            info (getId cmd) 2117
                "要想使用另一个用户来运行命令，请使用 su -c 或者 sudo。"
...
checkPrintfVar = CommandCheck (Exactly "printf") (f . arguments) where
    ...
    check format more = do
        sequence_ $ do
            ...
            在 POSIX sh 中，未定义 let。 formats = getPrintfFormats string
            在 POSIX sh 中，未定义 let。 formatCount = length formats
            在 POSIX sh 中，未定义 let。 argCount = length more
            在 POSIX sh 中，未定义 let。 pluraliseIfMany word n = if n > 1 then word 在 POSIX sh 中，未定义 ++ 。 "s" else word
            ...
            return $ if
                ...
                | otherwise ->
                    warn (getId format) 2183 $
                        "This format string has " 在 POSIX sh 中，未定义 ++ 。 show formatCount 在 POSIX sh 中，未定义 ++ 。 " " 在 POSIX sh 中，未定义 ++ 。 pluraliseIfMany "variable" formatCount 在 POSIX sh 中，未定义 ++ 。
                        ", but is passed " 在 POSIX sh 中，未定义 ++ 。 show argCount 在 POSIX sh 中，未定义 ++ 。 pluraliseIfMany " argument" argCount 在 POSIX sh 中，未定义 ++ 。 "."
    ...
    regexBasedGetFormats rest =
        case matchRegex re rest of
            Just [width, precision, typ, rest, _] ->
                (if width == "*" then "*" else "") 在 POSIX sh 中，未定义 ++ 。
                (if precision == "*" then "*" else "") 在 POSIX sh 中，未定义 ++ 。
                typ 在 POSIX sh 中，未定义 ++ 。 getFormats rest
            Nothing -> take 1 rest 在 POSIX sh 中，未定义 ++ 。 getFormats rest
      where
...
prop_checkUuoeCmd6 = verifyNot checkUuoeCmd "echo \"在 POSIX sh 中，未定义 $(<文件)。\""
  ...
  where
    check t = sequence_ $ do
        ...
        return . warn (getId t) 2163 $
            "This does not export '" 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 "'. Remove $/${} for that, or use ${var?} to quiet."
  ...
  where
    ...
    dollarWarning t = sequence_ $ do
        ...
        return . warn (getId t) 2229 $
            "This does not read '" 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 "'. Remove $/${} for that, or use ${var?} to quiet."
...
getSingleUnmodifiedBracedString word =
    case getWordParts word of
        [T_DollarBraced _ _ l] ->
            在 POSIX sh 中，未定义 let。 contents = concat $ oversimplify l
...
checkAliasesUsesArgs = CommandCheck (Exactly "alias") (f . arguments)
  where
    ...
    f = mapM_ checkArg
    checkArg arg =
        在 POSIX sh 中，未定义 let。 string = getLiteralStringDef "_" arg in
  ...
  where
    ...
    checkArg arg | '=' `elem` concat (oversimplify arg) =
        forM_ (find (not . isLiteral) $ getWordParts arg) $
            \x -> warn (getId x) 2139 "这种展开只能在定义的时候进行，而非使用的时候进行展开。请考虑使用转义。"
    checkArg _ = return ()
...
checkUnsetGlobs = CommandCheck (Exactly "unset") (mapM_ check . arguments)
  where
    check arg =
        when (isGlob arg) $
            warn (getId arg) 2184 "将 unset 的参数引起来，这样它们就不会被 glob 展开。"
...
checkFindWithoutPath = CommandCheck (Basename "find") f
  where
    ...
    hasPath (first:rest) =
        在 POSIX sh 中，未定义 let。 flag = getLiteralStringDef "___" first in
...
checkTimeParameters = CommandCheck (Exactly "time") f
  where
    f (T_SimpleCommand _ _ (cmd:args:_)) =
        whenShell [Bash, Sh] $
            在 POSIX sh 中，未定义 let。 s = concat $ oversimplify args in
...
checkTimedCommand = CommandCheck (Exactly "time") f where
    f (T_SimpleCommand _ _ (c:args@(_:_))) =
        whenShell [Sh, Dash, BusyboxSh] $ do
            在 POSIX sh 中，未定义 let。 cmd = last args -- "time" is parsed with a command as argument
            when (isPiped cmd) $
...
prop_checkLocalScope1 = verify checkLocalScope "在 POSIX sh 中，未定义 local。 foo=3"
prop_checkLocalScope2 = verifyNot checkLocalScope "f() { 在 POSIX sh 中，未定义 local。 foo=3; }"
checkLocalScope = CommandCheck (Exactly "在 POSIX sh 中，未定义 local。") $ \t ->
    whenShell [Bash, Dash, BusyboxSh] $ do -- Ksh allows it, Sh doesn't support 在 POSIX sh 中，未定义 local。
        path <- getPathM t
        unless (any isFunctionLike path) $
            err (getId $ getCommandTokenOrThis t) 2168 "'在 POSIX sh 中，未定义 local。' is only valid in functions."

prop_checkMultipleDeclaring1 = verify (checkMultipleDeclaring "在 POSIX sh 中，未定义 local。") "q() { 在 POSIX sh 中，未定义 local。 readonly var=1; }"
prop_checkMultipleDeclaring2 = verifyNot (checkMultipleDeclaring "在 POSIX sh 中，未定义 local。") "q() { 在 POSIX sh 中，未定义 local。 var=1; }"
prop_checkMultipleDeclaring3 = verify (checkMultipleDeclaring "readonly") "readonly 在 POSIX sh 中，未定义 local。 foo=5"
prop_checkMultipleDeclaring4 = verify (checkMultipleDeclaring "export") "export readonly foo=5"
prop_checkMultipleDeclaring5 = verifyNot (checkMultipleDeclaring "在 POSIX sh 中，未定义 local。") "f() { 在 POSIX sh 中，未定义 local。 -r foo=5; }"
prop_checkMultipleDeclaring6 = verifyNot (checkMultipleDeclaring "在 POSIX sh 中，未定义 declare。") "在 POSIX sh 中，未定义 declare。 -rx foo=5"
prop_checkMultipleDeclaring7 = verifyNot (checkMultipleDeclaring "readonly") "readonly '在 POSIX sh 中，未定义 local。' foo=5"
  ...
  where
    check t = sequence_ $ do
        ...
        return $ err (getId $ getCommandTokenOrThis t) 2316 $
                 "This applies " 在 POSIX sh 中，未定义 ++ 。 cmd 在 POSIX sh 中，未定义 ++ 。 " to the variable named " 在 POSIX sh 中，未定义 ++ 。 lit 在 POSIX sh 中，未定义 ++ 。
                 ", which is probably not what you want. Use a separate command or the appropriate `在 POSIX sh 中，未定义 declare。` options instead."
...
checkWhileGetoptsCase = CommandCheck (Exactly "getopts") f
  where
    ...
    check optId opts id list = do
            unless (Nothing `M.member` handledMap) $ do
                ...
                unless (any (`M.member` handledMap) [Just "*",Just "?"]) $
                    warn id 2220 "无效标记未被处理。请添加一个 *) case 语句。"
    ...
    warnUnhandled optId caseId str =
        warn caseId 2213 $ "getopts specified -" 在 POSIX sh 中，未定义 ++ 。 (e4m str) 在 POSIX sh 中，未定义 ++ 。 ", but it's not handled by this 'case'."

    warnRedundant (Just str, expr)
        | str `notElem` ["*", ":", "?"] =
            warn (getId expr) 2214 "此 case 语句没有在 getopts 中指定。"
...
checkCatastrophicRm = CommandCheck (Basename "rm") $ \t ->
  ...
  where
        ...
        case getLiteralString token of
            Just str ->
                when (fixPath str `elem` importantPaths) $
                    warn (getId token) 2114 "警告:删除系统文件目录。"
            Nothing ->
                checkWord' token

    checkWord' token = sequence_ $ do
        filename <- getPotentialPath token
        在 POSIX sh 中，未定义 let。 path = fixPath filename
        return . when (path `elem` importantPaths) $
            warn (getId token) 2115 $ "Use \"${var:?}\" to ensure this never expands to " 在 POSIX sh 中，未定义 ++ 。 path 在 POSIX sh 中，未定义 ++ 。 " ."

    fixPath filename =
        在 POSIX sh 中，未定义 let。 normalized = skipRepeating '/' . skipRepeating '*' $ filename in
    ...
    getPotentialPath = getLiteralStringExt f
      where
        ...
        f (T_DollarBraced _ _ word) =
            在 POSIX sh 中，未定义 let。 var = onlyLiteralString word in
    ...
    paths = [
        "", "/bin", "/etc", "/home", "/mnt", "/usr", "/usr/share", "/usr/在 POSIX sh 中，未定义 local。",
        "/var", "/lib", "/dev", "/media", "/boot", "/lib64", "/usr/bin"
        ]
    importantPaths = filter (not . null) $
        ["", "/", "/*", "/*/*"] >>= (\x -> map (在 POSIX sh 中，未定义 ++ 。x) paths)


prop_checkLetUsage1 = verify checkLetUsage "在 POSIX sh 中，未定义 let。 a=1"
prop_checkLetUsage2 = verifyNot checkLetUsage "(( a=1 ))"
checkLetUsage = CommandCheck (Exactly "在 POSIX sh 中，未定义 let。") f
  where
    f t = whenShell [Bash,Ksh] $ do
        style (getId t) 2219 $ "Instead of '在 POSIX sh 中，未定义 let。 expr', prefer (( expr )) ."
...
checkMvArguments = CommandCheck (Basename "mv") $ missingDestination f
  where
    f t = err (getId t) 2224 "mv 命令没有跟随目标。请检查参数。"

checkCpArguments = CommandCheck (Basename "cp") $ missingDestination f
  where
    f t = err (getId t) 2225 "cp 命令没有跟随目标。请检查参数。"
    ...
    f t = do
        ...
        case redirecting of
            Just (T_Redirecting _ redirs@(_:_) (T_SimpleCommand _ _ args@(_:_:_))) -> do
                ...
                在 POSIX sh 中，未定义 let。 minRedir = minimum $ map getId redirs
                在 POSIX sh 中，未定义 let。 maxArg   = maximum $ map getId args
...
prop_checkSudoRedirect4 = verify checkSudoRedirect "sudo cmd 在 POSIX sh 中，未定义 &>。 file"
...
prop_checkSudoArgs3 = verifyNot checkSudoArgs "sudo ls /usr/在 POSIX sh 中，未定义 local。/protected"
...
checkSudoArgs = CommandCheck (Basename "sudo") f
  where
    f t = sequence_ $ do
        ...
        return $ warn (getId t) 2232 $ "Can't use sudo with builtins like " 在 POSIX sh 中，未定义 ++ 。 command 在 POSIX sh 中，未定义 ++ 。 ". Did you want sudo sh -c .. instead?"
...
checkChmodDashr = CommandCheck (Basename "chmod") f
  where
    ...
    check t = sequence_ $ do
        ...
        guard $ flag == "-r"
        return $ warn (getId t) 2253 "使用 -R 以递归，或者显式的使用 a-r 来删除读权限。"
...
checkXargsDashi = CommandCheck (Basename "xargs") f
  where
    f t = sequence_ $ do
        ...
        return $ info (getId option) 2267 "GNU xargs -i 已废弃，请使用 -I{}"
    parseOpts = getBsdOpts "0oprtxadR:S:J:L:l:n:P:s:e:E:i:I:"


prop_checkArgComparison1 = verify (checkArgComparison "在 POSIX sh 中，未定义 declare。") "在 POSIX sh 中，未定义 declare。 a = b"
prop_checkArgComparison2 = verify (checkArgComparison "在 POSIX sh 中，未定义 declare。") "在 POSIX sh 中，未定义 declare。 a =b"
prop_checkArgComparison3 = verifyNot (checkArgComparison "在 POSIX sh 中，未定义 declare。") "在 POSIX sh 中，未定义 declare。 a=b"
prop_checkArgComparison4 = verify (checkArgComparison "export") "export a 在 POSIX sh 中，未定义 +=。b"
prop_checkArgComparison7 = verifyNot (checkArgComparison "在 POSIX sh 中，未定义 declare。") "在 POSIX sh 中，未定义 declare。 -a +i foo"
prop_checkArgComparison8 = verify (checkArgComparison "在 POSIX sh 中，未定义 let。") "在 POSIX sh 中，未定义 let。 x = 0"
prop_checkArgComparison9 = verify (checkArgComparison "alias") "alias x =0"
-- This mirrors checkSecondArgIsComparison but for arguments to 在 POSIX sh 中，未定义 local。/readonly/在 POSIX sh 中，未定义 declare。/export
    ...
    check arg = do
      sequence_ $ do
        ...
        case str of
            ...
            '+':'=':_ ->
                return $ err (headId arg) 2290 $
                    "Remove spaces around 在 POSIX sh 中，未定义 +=。 to append."
            _ -> Nothing

       -- '在 POSIX sh 中，未定义 let。' is parsed as a sequence of arithmetic expansions,
       -- so we want the additional warning for "x="
      when (cmd == "在 POSIX sh 中，未定义 let。") $ sequence_ $ do
...
prop_checkMaskedReturns1 = verify (checkMaskedReturns "在 POSIX sh 中，未定义 local。") "f() { 在 POSIX sh 中，未定义 local。 a=$(false); }"
prop_checkMaskedReturns2 = verify (checkMaskedReturns "在 POSIX sh 中，未定义 declare。") "在 POSIX sh 中，未定义 declare。 a=$(false)"
prop_checkMaskedReturns3 = verify (checkMaskedReturns "在 POSIX sh 中，未定义 declare。") "在 POSIX sh 中，未定义 declare。 a=\"`false`\""
...
prop_checkMaskedReturns6 = verifyNot (checkMaskedReturns "在 POSIX sh 中，未定义 declare。") "在 POSIX sh 中，未定义 declare。 a; a=$(false)"
prop_checkMaskedReturns7 = verifyNot (checkMaskedReturns "在 POSIX sh 中，未定义 local。") "f() { 在 POSIX sh 中，未定义 local。 -r a=$(false); }"
...
prop_checkMaskedReturns14 = verify (checkMaskedReturns "在 POSIX sh 中，未定义 declare。") "在 POSIX sh 中，未定义 declare。 x=${ false; }"
prop_checkMaskedReturns15 = verify (checkMaskedReturns "在 POSIX sh 中，未定义 declare。") "f() { 在 POSIX sh 中，未定义 declare。 x=$(false); }"
  ...
  where
    checkCmd t = do
        ...
        sequence_ $ do
            ...
            在 POSIX sh 中，未定义 let。 flags = map snd (getAllFlags t)
            在 POSIX sh 中，未定义 let。 hasDashR =  "r" `elem` flags
            在 POSIX sh 中，未定义 let。 hasDashG =  "g" `elem` flags
            在 POSIX sh 中，未定义 let。 isInScopedFunction = any (isScopedFunction shell) path

            在 POSIX sh 中，未定义 let。 isLocal = not hasDashG && isLocalInFunction name && isInScopedFunction
            在 POSIX sh 中，未定义 let。 isReadOnly = name == "readonly" || hasDashR

            -- Don't warn about 在 POSIX sh 中，未定义 local。 variables that are 在 POSIX sh 中，未定义 declare。d readonly,
            -- because the workaround `在 POSIX sh 中，未定义 local。 x; x=$(false); 在 POSIX sh 中，未定义 local。 -r x;` is annoying
    ...
    checkArgs (T_Assignment id _ _ _ word) | any hasReturn $ getWordParts word =
        warn id 2155 "将声明和赋值分开以避免覆盖返回值。"
    checkArgs _ = return ()

    isLocalInFunction = (`elem` ["在 POSIX sh 中，未定义 local。", "在 POSIX sh 中，未定义 declare。", "typeset"])
    isScopedFunction shell t =
        case t of
            T_BatsTest {} -> True
            -- In ksh, only functions 在 POSIX sh 中，未定义 declare。d with 'function' have their own scope
...
checkUnquotedEchoSpaces = CommandCheck (Basename "echo") check
  where
    check t = do
        在 POSIX sh 中，未定义 let。 args = arguments t
        m <- asks tokenPositions
        redir <- getClosestCommandM t
        sequence_ $ do
            在 POSIX sh 中，未定义 let。 positions = mapMaybe (\c -> M.lookup (getId c) m) args
            在 POSIX sh 中，未定义 let。 pairs = zip positions (drop 1 positions)
            (T_Redirecting _ redirTokens _) <- redir
            在 POSIX sh 中，未定义 let。 redirPositions = mapMaybe (\c -> fst <$> M.lookup (getId c) m) redirTokens
            guard $ any (hasSpacesBetween redirPositions) pairs
            return $ info (getId t) 2291 "请将重复的空格引起以避免彼此折叠成为单个空格。"
  ...
  where
    check t =
        when (isArrayExpansion t) $
            if isEscaped t
            then style (getId t) 2293 "当对 @Q-引起的词语进行求值时，请使用 * 来代替 @ 作为索引。"
            else warn (getId t) 2294 "eval 语句的劣势抵消掉了数组的优势。去掉 eval 来保留空格或符号(或者作为字符串来进行 eval)。"
...
prop_checkBackreferencingDeclaration1 = verify (checkBackreferencingDeclaration "在 POSIX sh 中，未定义 declare。") "在 POSIX sh 中，未定义 declare。 x=1 y=foo$x"
prop_checkBackreferencingDeclaration2 = verify (checkBackreferencingDeclaration "readonly") "readonly x=1 y=$((1+x))"
prop_checkBackreferencingDeclaration3 = verify (checkBackreferencingDeclaration "在 POSIX sh 中，未定义 local。") "在 POSIX sh 中，未定义 local。 x=1 y=$(echo $x)"
prop_checkBackreferencingDeclaration4 = verify (checkBackreferencingDeclaration "在 POSIX sh 中，未定义 local。") "在 POSIX sh 中，未定义 local。 x=1 y[$x]=z"
prop_checkBackreferencingDeclaration5 = verify (checkBackreferencingDeclaration "在 POSIX sh 中，未定义 declare。") "在 POSIX sh 中，未定义 declare。 x=var $x=1"
prop_checkBackreferencingDeclaration6 = verify (checkBackreferencingDeclaration "在 POSIX sh 中，未定义 declare。") "在 POSIX sh 中，未定义 declare。 x=var $x=1"
prop_checkBackreferencingDeclaration7 = verify (checkBackreferencingDeclaration "在 POSIX sh 中，未定义 declare。") "在 POSIX sh 中，未定义 declare。 x=var $k=$x"
checkBackreferencingDeclaration cmd = CommandCheck (Exactly cmd) check
  where
    ...
    warnIfBackreferencing cfga backrefs l = do
        ...
        在 POSIX sh 中，未定义 let。 reused = M.intersection backrefs references
        mapM msg $ M.toList reused

    msg (name, id) = warn id 2318 $ "This assignment is used again in this '" 在 POSIX sh 中，未定义 ++ 。 cmd 在 POSIX sh 中，未定义 ++ 。 "', but won't have taken effect. Use two '" 在 POSIX sh 中，未定义 ++ 。 cmd 在 POSIX sh 中，未定义 ++ 。 "'s."

    findReferences cfga list = do
        在 POSIX sh 中，未定义 let。 graph = CF.graph cfga
        在 POSIX sh 中，未定义 let。 nodesMap = CF.tokenToNodes cfga
        在 POSIX sh 中，未定义 let。 nodes = S.unions $ map (\id -> M.findWithDefault S.empty id nodesMap) $ map getId $ list
        在 POSIX sh 中，未定义 let。 labels = mapMaybe (G.lab graph) $ S.toList nodes
        在 POSIX sh 中，未定义 let。 references = M.fromList $ concatMap refFromLabel labels
        return references
