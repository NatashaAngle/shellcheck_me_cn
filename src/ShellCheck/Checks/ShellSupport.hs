...
prop_checkBashisms2 = verify checkBashisms "[ foo 在 POSIX sh 中，未定义 -nt。 bar ]"
prop_checkBashisms3 = verify checkBashisms "echo $((i在 POSIX sh 中，未定义 ++ 。))"
...
prop_checkBashisms15 = verify checkBashisms "在 POSIX sh 中，未定义 let。 n在 POSIX sh 中，未定义 ++ 。"
prop_checkBashisms16 = verify checkBashisms "echo $RANDOM"
prop_checkBashisms17 = verify checkBashisms "echo $((RANDOM%6+1))"
prop_checkBashisms18 = verify checkBashisms "foo 在 POSIX sh 中，未定义 &>。 /dev/null"
...
prop_checkBashisms40 = verify checkBashisms "echo 在 POSIX sh 中，未定义 $(<文件)。"
prop_checkBashisms41 = verify checkBashisms "echo 在 POSIX sh 中，未定义 `<file`。"
 ...
 where
  ...
  kludge params = bashism
   where
    ...
    warnMsg id code s =
        if isDash
        then err  id code $ "In dash, " 在 POSIX sh 中，未定义 ++ 。 s 在 POSIX sh 中，未定义 ++ 。 " not supported."
        else warn id code $ "In POSIX sh, " 在 POSIX sh 中，未定义 ++ 。 s 在 POSIX sh 中，未定义 ++ 。 " undefined."
    ...
    bashism (T_Arithmetic id _) = warnMsg id 3006 "在 POSIX sh 中，未定义 standalone ((..))。 is"
    bashism (T_DollarBracket id _) = warnMsg id 3007 "在POSIX sh中，未定义 $[..] 来代替 $((..))。 is"
    bashism (T_SelectIn id _ _ _) = warnMsg id 3008 "select loops are"
    bashism (T_BraceExpansion id _) = warnMsg id 3009 "在POSIX sh中，未定义大括号扩展。 is"
    bashism (T_Condition id DoubleBracket _) =
        unless isBusyboxSh $ warnMsg id 3010 "在 POSIX sh 中，未定义 [[ ]]。 is"
    bashism (T_HereString id _) = warnMsg id 3011 "here-strings are"
    bashism (TC_Binary id SingleBracket op _ _)
        | op `elem` [ "<", ">", "\\<", "\\>", "<=", ">=", "\\<=", "\\>="] =
            unless isDash $ warnMsg id 3012 $ "lexicographical " 在 POSIX sh 中，未定义 ++ 。 op 在 POSIX sh 中，未定义 ++ 。 " is"
    bashism (T_SimpleCommand id _ [asStr -> Just "test", lhs, asStr -> Just op, rhs])
        | op `elem` [ "<", ">", "\\<", "\\>", "<=", ">=", "\\<=", "\\>="] =
            unless isDash $ warnMsg id 3012 $ "lexicographical " 在 POSIX sh 中，未定义 ++ 。 op 在 POSIX sh 中，未定义 ++ 。 " is"
    bashism (TC_Binary id SingleBracket op _ _)
        | op `elem` [ "-ot", "在 POSIX sh 中，未定义 -nt。", "-ef" ] =
            unless isDash $ warnMsg id 3013 $ op 在 POSIX sh 中，未定义 ++ 。 " is"
    bashism (T_SimpleCommand id _ [asStr -> Just "test", lhs, asStr -> Just op, rhs])
        | op `elem` [ "-ot", "在 POSIX sh 中，未定义 -nt。", "-ef" ] =
            unless isDash $ warnMsg id 3013 $ op 在 POSIX sh 中，未定义 ++ 。 " is"
    bashism (TC_Binary id SingleBracket "==" _ _) =
        unless isBusyboxSh $ warnMsg id 3014 "在 POSIX sh 中，未定义使用== 来代替 =。 is"
    bashism (T_SimpleCommand id _ [asStr -> Just "test", lhs, asStr -> Just "==", rhs]) =
        unless isBusyboxSh $ warnMsg id 3014 "在 POSIX sh 中，未定义使用== 来代替 =。 is"
    bashism (TC_Binary id SingleBracket "=~" _ _) =
            warnMsg id 3015 "在 POSIX sh 中，未定义 =~ 正则表达式匹配。 is"
    bashism (T_SimpleCommand id _ [asStr -> Just "test", lhs, asStr -> Just "=~", rhs]) =
            warnMsg id 3015 "在 POSIX sh 中，未定义 =~ 正则表达式匹配。 is"
    ...
    bashism (TC_Unary id _ "-a" _) =
            warnMsg id 3017 "在 POSIX sh 中，未定义使用一元运算符 -a 来代替 -e。 is"
    bashism (T_SimpleCommand id _ [asStr -> Just "test", asStr -> Just "-a", _]) =
            warnMsg id 3017 "在 POSIX sh 中，未定义使用一元运算符 -a 来代替 -e。 is"
    bashism (TA_Unary id op _)
        | op `elem` [ "|在 POSIX sh 中，未定义 ++ 。", "|--", "在 POSIX sh 中，未定义 ++ 。|", "--|"] =
            warnMsg id 3018 $ filter (/= '|') op 在 POSIX sh 中，未定义 ++ 。 " is"
    bashism (TA_Binary id "**" _ _) = warnMsg id 3019 "exponentials are"
    bashism (T_FdRedirect id "&" (T_IoFile _ (T_Greater _) _)) =
        unless isBusyboxSh $ warnMsg id 3020 "在 POSIX sh 中，未定义 &>。 is"
    bashism (T_FdRedirect id "" (T_IoFile _ (T_GREATAND _) file)) =
        unless (all isDigit $ onlyLiteralString file) $ warnMsg id 3021 "在 POSIX sh 中，未定义 >& filename（作为>& fd的相反项）。 is"
    bashism (T_FdRedirect id ('{':_) _) = warnMsg id 3022 "在 POSIX sh 中，未定义 命名文件描述符。 are"
    ...
    bashism t@(T_Pipe id "|&") =
        warnMsg id 3029 "在POSIX sh中， |&代替 2>&1 |未定义。 is"
    ...
    bashism (T_CoProc id _ _) =
        warnMsg id 3032 "在POSIX sh中，coproc 未定义。 is"

    bashism (T_Function id _ _ str _) | not (isVariableName str) =
        warnMsg id 3033 "在POSIX sh中， [a-zA-Z_][a-zA-Z0-9_]*之外的命名函数未定义。 is"

    bashism (T_DollarExpansion id [x]) | isOnlyRedirection x =
        warnMsg id 3034 "在 POSIX sh 中，未定义 $(<文件)。 to read files is"
    bashism (T_Backticked id [x]) | isOnlyRedirection x =
        warnMsg id 3035 "在 POSIX sh 中，未定义 `<file`。 to read files is"
    ...
    bashism t@(T_SimpleCommand id _ _)
        | t `isCommand` "在 POSIX sh 中，未定义 let。" = warnMsg id 3039 "'在 POSIX sh 中，未定义 let。' is"
        ...
        checkFlags (flag@(fid, flag'):rest)
            | startsOption flag' = do
                unless (flag' `matches` validFlagsRegex) $
                  forM_ (tail flag') $ \在 POSIX sh 中，未定义 let。ter ->
                    when (在 POSIX sh 中，未定义 let。ter `notElem` optionsSet) $
                      warnMsg fid 3041 $ "set flag " <> ('-':在 POSIX sh 中，未定义 let。ter:" is")
                checkOptions rest
    ...
    bashism t@(T_SimpleCommand id _ (cmd:rest)) =
        在 POSIX sh 中，未定义 let。 name = fromMaybe "" $ getCommandName t
            flags = getLeadingFlags t
        in do
            when (name == "在 POSIX sh 中，未定义 local。" && not isDash) $
                -- This is so commonly accepted that we'll make it a special case
                warnMsg id 3043 $ "'在 POSIX sh 中，未定义 local。' is"
            when (name `elem` unsupportedCommands) $
                warnMsg id 3044 $ "'" 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 "' is"
            sequence_ $ do
                allowed' <- Map.lookup name allowedFlags
                allowed <- allowed'
                (word, flag) <- find
                    (\x -> (not . null . snd $ x) && snd x `notElem` allowed) flags
                return . warnMsg (getId word) 3045 $ name 在 POSIX sh 中，未定义 ++ 。 " -" 在 POSIX sh 中，未定义 ++ 。 flag 在 POSIX sh 中，未定义 ++ 。 " is"
            ...
            when (name == "trap") $
                在 POSIX sh 中，未定义 let。
                    check token = sequence_ $ do
                        ...
                        在 POSIX sh 中，未定义 let。 upper = map toUpper str
                        return $ do
                            when (upper `elem` ["ERR", "DEBUG", "RETURN"]) $
                                warnMsg (getId token) 3047 $ "trapping " 在 POSIX sh 中，未定义 ++ 。 str 在 POSIX sh 中，未定义 ++ 。 " is"
                            when (not isBusyboxSh && "SIG" `isPrefixOf` upper) $
                                warnMsg (getId token) 3048
                                    "在 POSIX sh 中，未定义在 SIG 中使用前缀信号名称。 is"
                            when (not isDash && upper /= str) $
                                warnMsg (getId token) 3049
                                    "在 POSIX sh 中，未定义使用小写或者混合大小写的信号名称。 is"
                in
                    mapM_ check (drop 1 rest)

            when (name == "printf") $ sequence_ $ do
                format <- rest !!! 0  -- flags are covered by allowedFlags
                在 POSIX sh 中，未定义 let。 literal = onlyLiteralString format
                guard $ "%q" `isInfixOf` literal
                return $ warnMsg (getId format) 3050 "在 POSIX sh 中，未定义 printf %q。 is"

            when (name == "read" && all isFlag rest) $
                warnMsg (getId cmd) 3061 "read without a variable is"
      where
        unsupportedCommands = [
            "在 POSIX sh 中，未定义 let。", "caller", "builtin", "comp在 POSIX sh 中，未定义 let。e", "compgen", "在 POSIX sh 中，未定义 declare。", "dirs", "disown",
    ...
    varChars="_0-9a-zA-Z"
    advancedExpansions = 在 POSIX sh 中，未定义 let。 re = mkRegex in [
        (re $ "^![" 在 POSIX sh 中，未定义 ++ 。 varChars 在 POSIX sh 中，未定义 ++ 。 "]", 3053, "在 POSIX sh 中，未定义间接展开。 is"),
        (re $ "^[" 在 POSIX sh 中，未定义 ++ 。 varChars 在 POSIX sh 中，未定义 ++ 。 "]+\\[.*\\]$", 3054, "array references are"),
        (re $ "^![" 在 POSIX sh 中，未定义 ++ 。 varChars 在 POSIX sh 中，未定义 ++ 。 "]+\\[[*@]]$", 3055, "在 POSIX sh 中，未定义数组键展开。 is"),
        (re $ "^![" 在 POSIX sh 中，未定义 ++ 。 varChars 在 POSIX sh 中，未定义 ++ 。 "]+[*@]$", 3056, "name matching prefixes are"),
        (re $ "^[" 在 POSIX sh 中，未定义 ++ 。 varChars 在 POSIX sh 中，未定义 ++ 。 "*@]+(\\[.*\\])?[,^]", 3059, "case modification is")
        ]
    simpleExpansions = 在 POSIX sh 中，未定义 let。 re = mkRegex in [
        (re $ "^[" 在 POSIX sh 中，未定义 ++ 。 varChars 在 POSIX sh 中，未定义 ++ 。 "*@]+:[^-=?+]", 3057, "在 POSIX sh 中，未定义字符串索引。 is"),
        (re $ "^([*@][%#]|#[@*])", 3058, "string operations on $@/$* are"),
        (re $ "^[" 在 POSIX sh 中，未定义 ++ 。 varChars 在 POSIX sh 中，未定义 ++ 。 "*@]+(\\[.*\\])?/", 3060, "在 POSIX sh 中，未定义字符串替换。 is")
        ]
    bashVars = [
...
checkEchoSed = ForShell [Bash, Ksh] f
  where
    ...
    isSimpleSed s = isJust $ do
        ...
        在 POSIX sh 中，未定义 let。 delimiters = filter (== h) rest
        guard $ length delimiters == 2
    checkIn id s =
        when (isSimpleSed s) $
            style id 2001 "请确认是否可改用 ${variable//search/replace} 。"
      ...
      where
        check element =
            when (any (`isInfixOf` toString element) ["$..", "..$"]) $ do
                c <- isEvaled element
                if c
                    then style id 2175 "将此无效的花括号扩展用引号引起，因为 eval 只能操作传递进去的文字信息."
                    else warn id 2051 "Bash 在大括号范围展开中不支持变量。"
    f _ = return ()
  ...
  where
    f token =
        case token of
            ...
            _ -> return ()
    about t = warn (getId t) 2180 "Bash 不支持多维数组。请使用一维数组或者关联数组。"
