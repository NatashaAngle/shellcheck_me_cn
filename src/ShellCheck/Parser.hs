...
quotableChars = "|&;<>()\\ '\t\n\r\xA0" 在 POSIX sh 中，未定义 ++ 。 doubleQuotableChars
  ...
  where
    continuation = do
        ...
        optional $ do
            x <- readComment
            when ("\\" `isSuffixOf` x) $
                parseProblem ErrorC 1143 "此反斜杠是注释的一部分，因此不会继续当前行。"
        return whitespace
...
allspacing = do
    ...
    if more then do
        rest <- allspacing
        return $ s 在 POSIX sh 中，未定义 ++ 。 "\n" 在 POSIX sh 中，未定义 ++ 。 rest
      else
        return s
...
readUnicodeQuote = do
    start <- startSpan
    c <- oneOf (unicodeSingleQuotes 在 POSIX sh 中，未定义 ++ 。 unicodeDoubleQuotes)
    id <- endSpan start
    parseProblemAtId id WarningC 1110 "这是 Unicode 引号。删除并重新输入英文半角引号(或加引号使其为文字)。"
...
almostSpace =
  ...
  where
    check c name = do
        parseNote ErrorC 1018 $ "This is a " 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 ". De在 POSIX sh 中，未定义 let。e and retype it."
        char c
        return ' '
...
getNextIdBetween startPos endPos = do
    state <- getState
    在 POSIX sh 中，未定义 let。 newId = incId (lastId state)
    在 POSIX sh 中，未定义 let。 newMap = Map.insert newId (startPos, endPos) (positionMap state)
    putState $ state {
        lastId = newId,
        positionMap = newMap
    }
...
data Incomp在 POSIX sh 中，未定义 let。eInterval = Incomp在 POSIX sh 中，未定义 let。eInterval SourcePos

startSpan = Incomp在 POSIX sh 中，未定义 let。eInterval <$> getPosition

endSpan (Incomp在 POSIX sh 中，未定义 let。eInterval start) = do
    endPos <- getPosition
...
addToHereDocMap id list = do
    state <- getState
    在 POSIX sh 中，未定义 let。 map = hereDocMap state
    putState $ state {
...
addPendingHereDoc id d q str = do
    ...
    在 POSIX sh 中，未定义 let。 docs = pendingHereDocs state
    putState $ state {
...
popPendingHereDocs = do
    state <- getState
    在 POSIX sh 中，未定义 let。 pending = pendingHereDocs state
    putState $ state {
        pendingHereDocs = []
    }
...
unexpecting s p = try $
    (try p >> fail ("Unexpected " 在 POSIX sh 中，未定义 ++ 。 s)) <|> return ()
  ...
  where
    ...
    guardArithmetic = do
        ...
        parseProblem ErrorC 1076 $
            if single
            then "尝试用数学？使用例如这样的代码：[ $((i/2+7)) -ge 18 ]。"
    ...
    readCondUnaryExp = do
      ...
      liftM op readCondWord `orFail` do
          parseProblemAt pos ErrorC 1019 "此处应是一元条件的参数。"
    ...
    readOp = try $ do
        char '-' <|> weirdDash
        s <- many1 在 POSIX sh 中，未定义 let。ter <|> fail "Expected a test operator"
        return ('-':s)
    ...
    readCondWord = do
        ...
        when (notArrayIndex x && endedWith "]" x && not (x `containsLiteral` "[")) $ do
            parseProblemAt pos ErrorC 1020 $
                "You need a space before the " 在 POSIX sh 中，未定义 ++ 。 (if single then "]" else "]]") 在 POSIX sh 中，未定义 ++ 。 "."
    ...
    readCondNullaryOrBinary = do
      ...
      (do
            ...
            y <- if isRegex
                    then readRegex
                    else  readCondWord <|> (parseProblemAt pos ErrorC 1027 "此操作符需要另外一个参数。" >> mzero)
            return (x `op` y)
          ) <|> ( do
            checkTrailingOp x
    ...
    checkTrailingOp x = sequence_ $ do
        ...
        return $ parseProblemAtId id ErrorC 1108 $
            "You need a space before and after the " 在 POSIX sh 中，未定义 ++ 。 trailingOp 在 POSIX sh 中，未定义 ++ 。 " ."
    ...
    readCondGroup = do
      ...
      where
        ...
        doubleWarning pos =
            parseProblemAt pos ErrorC 1029 "在 [[..]] 中不应转义 ( 或 )。"
    ...
    readRegex = called "regex" $ do
      ...
      where
        ...
        readGroup = called "regex grouping" $ do
            ...
            id <- endSpan start
            return $ T_NormalWord id (p1:(parts 在 POSIX sh 中，未定义 ++ 。 [p2]))
        readRegexLiteral = do
            start <- startSpan
...
prop_a1 = isOk readArithmeticContents " n在 POSIX sh 中，未定义 ++ 。 + 在 POSIX sh 中，未定义 ++ 。c"
...
prop_a13 = isOk readArithmeticContents "foo[9*y+x]在 POSIX sh 中，未定义 ++ 。"
...
readArithmeticContents =
  ...
  where
    spacing =
        在 POSIX sh 中，未定义 let。 lf = try (string "\\\n") >> return '\n'
    ...
    readComboOp op token = do
        ...
        op <- choice (map (\x -> try $ do
                                        s <- string x
                                        failIfIncomp在 POSIX sh 中，未定义 let。eOp
                                        return s
                            ) op)
        id <- endSpan start
        spacing
    ...
    failIfIncomp在 POSIX sh 中，未定义 let。eOp = notFollowedBy2 $ oneOf "&|<>="
    ...
    readMinusOp = do
        ...
        optional $ do
            ...
            parseProblemAt pos ErrorC 1106 $ "In arithmetic contexts, use " 在 POSIX sh 中，未定义 ++ 。 alt 在 POSIX sh 中，未定义 ++ 。 " instead of -" 在 POSIX sh 中，未定义 ++ 。 str
        id <- endSpan start
        spacing
        return $ TA_Binary id "-"
      where
    ...
    readAssignmentOp = readComboOp ["=", "*=", "/=", "%=", "在 POSIX sh 中，未定义 +=。", "-=", "<<=", ">>=", "&=", "^=", "|="] TA_Assignment
    ...
    readPrefixIncremented = do
        start <- startSpan
        op <- try $ string "在 POSIX sh 中，未定义 ++ 。" <|> string "--"
        id <- endSpan start
        spacing
        x <- readArithTerm
        return $ TA_Unary id (op 在 POSIX sh 中，未定义 ++ 。 "|") x
...
readCondition = called "test expression" $ do
    ...
    在 POSIX sh 中，未定义 let。 single = open == "["
    在 POSIX sh 中，未定义 let。 typ = if single then SingleBracket else DoubleBracket

    pos <- getPosition
    space <- allspacing
    when (null space) $
        parseProblemAtWithEnd opos pos ErrorC 1035 $ "You need a space after the " 在 POSIX sh 中，未定义 ++ 。
            if single
                then "[ and before the ]."
                else "[[ and before the ]]."
    when (single && '\n' `elem` space) $
    ...
    id <- endSpan start
    when (open == "[[" && close /= "]]") $ parseProblemAt cpos ErrorC 1033 "测试表达式使用双 [[ 开始但却使用单个 ] 闭合。请确保它们能匹配。"
...
readAnnotationWithoutPrefix sandboxed = do
  ...
  where
    ...
    readKey = do
        keyPos <- getPosition
        key <- many1 (在 POSIX sh 中，未定义 let。ter <|> char '-')
        ...
        annotations <- case key of
            ...
            "enable" -> plainOrQuoted $ readName `sepBy` char ','
              where
                readName = EnableComment <$> many1 (在 POSIX sh 中，未定义 let。ter <|> char '-')
            ...
            "shell" -> do
                ...
                when (isNothing $ shellForExecutable shell) $
                    parseNoteAt pos ErrorC 1103
                        "shell类型不可识别。请使用例如sh 或 bash。"
                return [ShellOverride shell]

            "extended-analysis" -> do
                pos <- getPosition
                value <- plainOrQuoted $ many1 在 POSIX sh 中，未定义 let。ter
                case value of
            ...
            "external-sources" -> do
                ...
                case value of
                    "true" ->
                        if sandboxed
                        then do
                            parseNoteAt pos ErrorC 1144 "外部-源代码 只能在 .shellcheckrc 中启用，不能在单个文件中用。"
                            return []
                        else return [ExternalSources True]
                    "false" -> return [ExternalSources False]
                    _ -> do
                        parseNoteAt pos ErrorC 1145 "未知的外部-源代码值。需要 true/false。"
                        return []

            _ -> do
                parseNoteAt keyPos WarningC 1107 "该指令未知。它将被忽略。"
...
checkPossibleTermination pos [T_Literal _ x] terminators =
    when (x `elem` terminators) $
        parseProblemAt pos WarningC 1010 $ "Use semicolon or linefeed before '" 在 POSIX sh 中，未定义 ++ 。 x 在 POSIX sh 中，未定义 ++ 。 "' (or quote to make it literal)."
  ...
  where
    ...
    literalBraces = do
        ...
        parseProblemAt pos WarningC 1083 $
            "This " 在 POSIX sh 中，未定义 ++ 。 [c] 在 POSIX sh 中，未定义 ++ 。 " is literal. Check expression (missing ;/\\n?) or quote it."
        return [c]
...
readSingleQuoted = called "single quoted string" $ do
    ...
    在 POSIX sh 中，未定义 let。 string = concat s
    endPos <- getPosition
...
readSingleQuotedPart =
    readSingleEscaped
    <|> many1 (noneOf $ "'\\" 在 POSIX sh 中，未定义 ++ 。 unicodeSingleQuotes)
    <|> readUnicodeQuote
   where
    readUnicodeQuote = do
        pos <- getPosition
...
suggestForgotClosingQuote startPos endPos name = do
    parseProblemAt startPos WarningC 1078 $
        "Did you forget to close this " 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 "?"
...
readDoubleLiteralPart = do
    x <- many1 (readDoubleEscaped <|> many1 (noneOf (doubleQuotableChars 在 POSIX sh 中，未定义 ++ 。 unicodeDoubleQuotes)))
    return $ concat x
...
readGlob = readExtglob <|> readSimple <|> readClass <|> readGlobbyLiteral
    where
        ...
        readClass = try $ do
            ...
            char ']'
            id <- endSpan start
            return $ T_Glob id $ "[" 在 POSIX sh 中，未定义 ++ 。 concat (negation:leadingBracket:s) 在 POSIX sh 中，未定义 ++ 。 "]"
          where
           globchars = charToString $ oneOf $ "![" 在 POSIX sh 中，未定义 ++ 。 extglobStartChars
           predefined = do
              try $ string "[:"
              s <- many1 在 POSIX sh 中，未定义 let。ter
              string ":]"
              return $ "[:" 在 POSIX sh 中，未定义 ++ 。 s 在 POSIX sh 中，未定义 ++ 。 ":]"
...
readNormalLiteralPart customEnd =
    readNormalEscaped <|>
        many1 (noneOf (customEnd 在 POSIX sh 中，未定义 ++ 。 standardEnd))
  where
    standardEnd = "[{}"
        在 POSIX sh 中，未定义 ++ 。 quotableChars
        在 POSIX sh 中，未定义 ++ 。 extglobStartChars
        在 POSIX sh 中，未定义 ++ 。 unicodeDoubleQuotes
        在 POSIX sh 中，未定义 ++ 。 unicodeSingleQuotes
      ...
      <|>
        do
            ...
            case escapedChar next of
                Just name -> parseNoteAt pos WarningC 1012 $ "\\" 在 POSIX sh 中，未定义 ++ 。 [next] 在 POSIX sh 中，未定义 ++ 。 " is just literal '" 在 POSIX sh 中，未定义 ++ 。 [next] 在 POSIX sh 中，未定义 ++ 。 "' here. For " 在 POSIX sh 中，未定义 ++ 。 name 在 POSIX sh 中，未定义 ++ 。 ", use " 在 POSIX sh 中，未定义 ++ 。 alternative next 在 POSIX sh 中，未定义 ++ 。 " instead."
                Nothing -> parseNoteAt pos InfoC 1001 $ "This \\" 在 POSIX sh 中，未定义 ++ 。 [next] 在 POSIX sh 中，未定义 ++ 。 " will be a regular '" 在 POSIX sh 中，未定义 ++ 。 [next] 在 POSIX sh 中，未定义 ++ 。 "' in this context."
            return [next]
  where
    alternative 'n' = "a quoted, literal line feed"
    alternative t = "\"$(printf '\\" 在 POSIX sh 中，未定义 ++ 。 [t] 在 POSIX sh 中，未定义 ++ 。 "')\""
    ...
    checkTrailingSpaces pos = lookAhead . try $ do
        many linewhitespace
        void linefeed <|> eof
        parseProblemAt pos ErrorC 1101 "De在 POSIX sh 中，未定义 let。e trailing spaces after \\ to break line (or use quotes for literal space)."
...
readDollarExp = arithmetic <|> readDollarExpansion <|> readDollarBracket <|> readDollarBraceCommandExpansion <|> readDollarBraced <|> readDollarVariable
  where
    arithmetic = readAmbiguous "$((" readDollarArithmetic readDollarExpansion (\pos ->
        parseNoteAt pos ErrorC 1102 "Shell 以不同方式消除 $(( 歧义或根本不消除歧义。对 $(命令替换)，在$(后面添加空格。对 $((算数))，修复解析错误。")
...
readDollarVariable = do
    ...
    在 POSIX sh 中，未定义 let。 singleCharred p = do
        value <- wrapString ((:[]) <$> p)
        id <- endSpan start
        return $ (T_DollarBraced id False value)

    在 POSIX sh 中，未定义 let。 positional = do
        value <- singleCharred digit
        return value `attempting` do
            lookAhead digit
            parseNoteAt pos ErrorC 1037 "大于 9 (例如 ${10})的位置参数需要大括号。"

    在 POSIX sh 中，未定义 let。 special = singleCharred specialVariable

    在 POSIX sh 中，未定义 let。 regular = do
        value <- wrapString readVariableName
        id <- endSpan start
        return (T_DollarBraced id False value) `attempting` do
            lookAhead $ char '['
            parseNoteAt pos ErrorC 1087 "展开数组时请使用大括号，例如: ${array[idx]} (或 ${var}[.. 以静默)。"
...
readHereDoc = called "here document" $ do
    ...
    optional $ do
        try . lookAhead $ char '('
        在 POSIX sh 中，未定义 let。 message = "Shells are space sensitive. Use '< <(cmd)', not '<<" 在 POSIX sh 中，未定义 ++ 。 sp 在 POSIX sh 中，未定义 ++ 。 "(cmd)'."
    ...
    在 POSIX sh 中，未定义 let。 doc = T_HereDoc hid dashed quoted endToken []
  ...
  where
    ...
    readToken = do
        ...
        crstr <- (carriageReturn >> (return $ str 在 POSIX sh 中，未定义 ++ 。 "\r")) <|> return str
  ...
  where
    readDoc (HereDocPending id dashed quoted endToken ctx) =
      ...
      do
        ...
        在 POSIX sh 中，未定义 let。 hereData = unlines lines
    ...
    readDocLines dashed endToken = do
      ...
      where
        ...
        checkEnd = option (False, False) $ try $ do
            ...
            在 POSIX sh 中，未定义 let。 leadingSpacesAreTabs = all (== '\t') leadingSpace
            在 POSIX sh 中，未定义 let。 thereIsNoTrailer = null trailingSpace && null trailer
            在 POSIX sh 中，未定义 let。 leaderIsOk = null leadingSpace
                    || dashed == Dashed && leadingSpacesAreTabs
            在 POSIX sh 中，未定义 let。 trailerStart = case trailer of [] -> '\0'; (h:_) -> h
            在 POSIX sh 中，未定义 let。 hasTrailingSpace = not $ null trailingSpace
            在 POSIX sh 中，未定义 let。 hasTrailer = not $ null trailer
            在 POSIX sh 中，未定义 let。 ppt = parseProblemAt trailerPos ErrorC

            if leaderIsOk && thereIsNoTrailer
              then return (True, False)
              else do
                在 POSIX sh 中，未定义 let。 foundCause = return (False, True)
                在 POSIX sh 中，未定义 let。 skipLine = return (False, False)
                ...
                if
                    | trailerStart == ')' -> do
                        ...
                        foundCause
                    | trailerStart == '#' -> do
                        ppt 1120 "here 文档标记后面不允许注释。请改为注释下一行。"
                        foundCause
                    | trailerStart `elem` ";>|&" -> do
                        ppt 1121 "在含有 << 的行中添加 ;/& 终止符(和其他语法)，而不是在这里添加。"
                        foundCause
                    | hasTrailingSpace && hasTrailer -> do
                        ppt 1122 "结束标记后面不允许有任何内容。要继续一条命令，请将其放在包含 << 的行中。"
                        foundCause
                    | leaderIsOk && hasTrailingSpace && not hasTrailer -> do
                        parseProblemAt trailingSpacePos ErrorC 1118 "请删除 here 文档结束标记后的空格。"
                    ...
                    | dashed == Undashed && (not $ null leadingSpace) -> do
                        parseProblemAt leadingSpacePos ErrorC 1039 "移除结束标记前的缩进(或使用 <<- 并以制表符缩进)。"
                        foundCause
                    | dashed == Dashed && not leadingSpacesAreTabs -> do
                        parseProblemAt leadingSpacePos ErrorC 1040 "使用 <<- 时，只能使用制表符缩进。"
                        foundCause
                    | True -> skipLine
    ...
    debugHereDoc tokenId endToken doc
        | endToken `isInfixOf` doc =
            在 POSIX sh 中，未定义 let。 lookAt line = when (endToken `isInfixOf` line) $
                      parseProblemAtId tokenId ErrorC 1042 ("Close matches include '" 在 POSIX sh 中，未定义 ++ 。 (e4m line) 在 POSIX sh 中，未定义 ++ 。 "' (!= '" 在 POSIX sh 中，未定义 ++ 。 (e4m endToken) 在 POSIX sh 中，未定义 ++ 。 "').")
            in do
                  parseProblemAtId tokenId ErrorC 1041 ("Found '" 在 POSIX sh 中，未定义 ++ 。 (e4m endToken) 在 POSIX sh 中，未定义 ++ 。 "' further down, but not on a separate line.")
                  mapM_ lookAt (lines doc)
        | map toLower endToken `isInfixOf` map toLower doc =
            parseProblemAtId tokenId ErrorC 1043 ("Found " 在 POSIX sh 中，未定义 ++ 。 (e4m endToken) 在 POSIX sh 中，未定义 ++ 。 " further down, but with wrong casing.")
        | otherwise =
            parseProblemAtId tokenId ErrorC 1044 ("Couldn't find end token `" 在 POSIX sh 中，未定义 ++ 。 (e4m endToken) 在 POSIX sh 中，未定义 ++ 。 "' in the here document.")
...
readIoDuplicate = try $ do
  ...
  where
    ...
    digitsAndOrDash = do
        ...
        return $ str 在 POSIX sh 中，未定义 ++ 。 dash
...
readIoVariable = try $ do
    ...
    x <- readVariableName
    char '}'
    return $ "{" 在 POSIX sh 中，未定义 ++ 。 x 在 POSIX sh 中，未定义 ++ 。 "}"
...
prop_readIoRedirect4 = isOk readIoRedirect "在 POSIX sh 中，未定义 &>。 lol"
...
prop_readNewlineList1 = isOk readScript "在 POSIX sh 中，未定义 &>。 /dev/null echo foo"
readNewlineList =
  ...
  where
    checkBadBreak = optional $ do
                ...
                notFollowedBy2 (string "在 POSIX sh 中，未定义 &>。") --  Except 在 POSIX sh 中，未定义 &>。 or 在 POSIX sh 中，未定义 &>。> which is valid
                parseProblemAt pos ErrorC 1133
                    "意外的新行首。如需断行，|，|| 或者 && 应放在上一行的末尾。"
...
readSeparatorOp = do
    ...
    notFollowedBy2 (string "在 POSIX sh 中，未定义 &>。")
    start <- getPosition
    f <- try (do
                    pos <- getPosition
                    char '&'
                    optional $ choice [
                        do
                            s <- lookAhead . choice . map (try . string) $
                                ["amp;", "gt;", "lt;"]
                            parseProblemAt pos ErrorC 1109 "这是未加引号的 HTML 实体。请替换为对应的字符。",

                        do
                            try . lookAhead $ variableStart
                            parseProblemAt pos WarningC 1132 "此 & 会终止命令。请将其转义或在 & 后面添加空格以静默。"
                      ]
...
prop_readSimpleCommand2 = isOk readSimpleCommand "cmd 在 POSIX sh 中，未定义 &>。 file"
...
readSimpleCommand = called "simple command" $ do
      ...
      Just cmd -> do
            ...
            suffix <- option [] $ getParser readCmdSuffix
                    ...
                    (if isCommand ["builtin"] cmd then fromMaybe cmd firstArgument else cmd) [
                        (["在 POSIX sh 中，未定义 declare。", "export", "在 POSIX sh 中，未定义 local。", "readonly", "typeset"], readModifierSuffix),
                        (["time"], readTimeSuffix),
                        (["在 POSIX sh 中，未定义 let。"], readLetSuffix),
                        (["eval"], readEvalSuffix)
                    ]

            id1 <- getNextIdSpanningTokenList (prefix 在 POSIX sh 中，未定义 ++ 。 (cmd:suffix))
            id2 <- getNewIdFor id1

            在 POSIX sh 中，未定义 let。 result = makeSimpleCommand id1 id2 prefix [cmd] suffix
            case () of
  ...
  where
    ...
    validateCommand cmd =
        case cmd of
            ...
            (T_NormalWord _ (T_Literal _ str:_)) -> do
                在 POSIX sh 中，未定义 let。 cmdString = map toLower $ takeWhile isAlpha str
    ...
    commentWarning id =
        parseProblemAtId id ErrorC 1127 "您是想要创建注释吗？请在 sh 中使用 #。"
    ...
    makeSimpleCommand id1 id2 prefix cmd suffix =
        在 POSIX sh 中，未定义 let。
            ...
            redirs = preRedirected 在 POSIX sh 中，未定义 ++ 。 postRedirected
            assigns = preAssigned
            args = cmd 在 POSIX sh 中，未定义 ++ 。 preRest2 在 POSIX sh 中，未定义 ++ 。 postRest
        in
...
readSource t@(T_Redirecting _ _ (T_SimpleCommand cmdId _ (cmd:file':rest'))) = do
    在 POSIX sh 中，未定义 let。 file = getFile file' rest'
    override <- getSourceOverride
    在 POSIX sh 中，未定义 let。 literalFile = do
              ...
              then do
                ...
                (input, resolvedFile) <-
                    ...
                    then return (Right "", filename)
                        ...
                        在 POSIX sh 中，未定义 let。 paths = mapMaybe getSourcePath allAnnotations
                        在 POSIX sh 中，未定义 let。 externalSources = listToMaybe $ mapMaybe getExternalSources allAnnotations
...
readAndOr = do
    ...
    unless (null annotations) $ optional $ do
        ...
        parseProblemAt apos ErrorC 1123 "ShellCheck directives are only valid in front of comp在 POSIX sh 中，未定义 let。e compound commands, like 'if', not e.g. individual 'elif' branches."
...
readPipeSequence = do
  ...
  where
    sepBy1WithSeparators p s = do
        在 POSIX sh 中，未定义 let。 elems = (\x -> ([x], [])) <$> p
        在 POSIX sh 中，未定义 let。 seps = do
            separator <- s
            return $ \(a,b) (c,d) -> (a在 POSIX sh 中，未定义 ++ 。c, b 在 POSIX sh 中，未定义 ++ 。 d 在 POSIX sh 中，未定义 ++ 。 [separator])
        elems `chainl1` seps
...
skipAnnotationAndWarn = optional $ do
        ...
        parseProblem ErrorC 1126 "请将 ShellCheck 指令放在命令前面，而不是后面。"
        readAnyComment
...
verifyNotEmptyIf s =
    optional (do
                ...
                parseProblemAt emptyPos ErrorC 1048 $ "Can't have empty " 在 POSIX sh 中，未定义 ++ 。 s 在 POSIX sh 中，未定义 ++ 。 " clauses (use 'true' as a no-op).")
readIfPart = do
    pos <- getPosition
    g_If
    allspacing
...
readBatsTest = called "bats @test" $ do
  ...
  where
    readBatsName = do
        ...
        在 POSIX sh 中，未定义 let。 name = reverse $ f $ reverse line
        string name
...
readDoGroup kwId = do
    ...
    acceptButWarn g_Semi ErrorC 1059 "Semicolon is not allowed directly after 'do'. You can just de在 POSIX sh 中，未定义 let。e it."
    allspacing
...
prop_readForClause4 = isOk readForClause "for((i=0; i<10; i在 POSIX sh 中，未定义 ++ 。)); do echo $i; done"
prop_readForClause5 = isOk readForClause "for ((i=0;i<10 && n>x;i在 POSIX sh 中，未定义 ++ 。,--n))\ndo \necho $i\ndone"
  ...
  where
    ...
    readArithmeticDelimiter c msg = do
        ...
        char c <|> do
            ...
            fail ""
        unless (null sp) $
            parseProblemAtWithEnd startPos endPos ErrorC 1138 $ "Remove spaces between " 在 POSIX sh 中，未定义 ++ 。 [c,c] 在 POSIX sh 中，未定义 ++ 。 " in arithmetic for loop."
...
readCaseItem = called "case item" $ do
    ...
    optional $ do
        try . lookAhead $ readAnnotationPrefix
        parseProblem ErrorC 1124 "ShellCheck directives are only valid in front of comp在 POSIX sh 中，未定义 let。e commands like 'case' statements, not individual case branches."
    optional g_Lparen
    spacing
    pattern' <- readPattern
    void g_Rparen <|> do
...
readFunctionDefinition = called "function" $ do
    ...
    allspacing
    void (lookAhead $ oneOf "{(") <|> parseProblem ErrorC 1064 "应使用 { 来开启函数定义。"
    ...
    readFunctionSignature =
      ...
      where
        readWithFunction = do
            ...
            when (not hasParens && null spaces) $
                acceptButWarn (lookAhead (oneOf "{("))
                    ErrorC 1095 "您需要在函数名称和函数体之间插入空格或换行符。"
        ...
        readParens = do
            ...
            spacing
            g_Rparen <|> do
                parseProblem ErrorC 1065 "Trying to 在 POSIX sh 中，未定义 declare。 parameters? Don't. Use () and refer to params as $1, $2.."
                many $ noneOf "\n){"
                g_Rparen
            return ()

prop_readCoProc1 = isOk readCoProc "在POSIX sh中，coproc 未定义。 foo { echo bar; }"
prop_readCoProc2 = isOk readCoProc "在POSIX sh中，coproc 未定义。 { echo bar; }"
prop_readCoProc3 = isOk readCoProc "在POSIX sh中，coproc 未定义。 echo bar"
readCoProc = called "在POSIX sh中，coproc 未定义。" $ do
    start <- startSpan
    try $ do
        string "在POSIX sh中，coproc 未定义。"
        whitespace
...
readConditionCommand = do
    ...
    hasDashAo <- isFollowedBy $ do
        ...
        parseProblemAtWithEnd pos posEnd ErrorC 1139 $
            "Use " 在 POSIX sh 中，未定义 ++ 。 alt c 在 POSIX sh 中，未定义 ++ 。 " instead of '" 在 POSIX sh 中，未定义 ++ 。 c 在 POSIX sh 中，未定义 ++ 。 "' between test commands."
    ...
    when (hasWord && not (hasKeyword || hasDashAo)) $ do
        ...
        posEnd <- getPosition
        parseProblemAtWithEnd pos posEnd ErrorC 1140 "条件之后出现意外的参数。缺少 &&，|| 或者表达式错误？"
...
readCompoundCommand = do
    ...
    optional . lookAhead $ do
        ...
        posEnd <- getPosition
        parseProblemAtWithEnd pos posEnd ErrorC 1141 "在复合命令后意外出现标识。重定向错误或者缺少 ;，&&，|| 或者 |？"
    return $ T_Redirecting id redirs cmd
...
readTimeSuffix = do
    ...
    return $ flags 在 POSIX sh 中，未定义 ++ 。 [pipeline]
  where
...
-- Fixme: this is a hack that doesn't handle 在 POSIX sh 中，未定义 let。 c='4'"5" or 在 POSIX sh 中，未定义 let。 a\>b
...
readLetSuffix = many1 (readIoRedirect <|> try readLetExpression <|> readCmdWord)
  where
    ...
    readLetExpression = do
        ...
        在 POSIX sh 中，未定义 let。 (unQuoted, newPos) = kludgeAwayQuotes expression startPos
    ...
    kludgeAwayQuotes s p =
        case s of
            first:second:rest ->
                在 POSIX sh 中，未定义 let。 (last NE.:| backwards) = NE.reverse (second NE.:| rest)
...
prop_readAssignmentWord5 = isOk readAssignmentWord "b在 POSIX sh 中，未定义 +=。lol"
...
readAssignmentWordExt lenient = called "variable assignment" $ do
        ...
        when (isJust leadingDollarPos || hasLeftSpace) $ do
            ...
            when hasParen $
                sequence_ $ do
                    ...
                    return $ parseProblemAtWithEnd l r ErrorC 1066 "请勿在赋值的左侧使用 $。"
    ...
    if hasRightSpace || isEndOfCommand
      then do
        ...
        optional $ do
            lookAhead $ char '='
            parseProblem ErrorC 1097 "意外 ==。对于赋值语句，请使用 =。对于比较，请使用 [ 或 [[。 Or quote for literal string."
  ...
  where
    readAssignmentOp = do
        ...
        unexpecting "===" (string "===")
        choice [
            string "在 POSIX sh 中，未定义 +=。" >> return Append,
...
readArray = called "array assignment" $ do
    ...
    optional $ do
        lookAhead $ char '('
        parseProblemAt opening ErrorC 1116 "$((..)) 表达式中缺少 $？(如果是数组，请使用 ( ()。"
    allspacing
...
tryParseWordToken keyword t = try $ do
    ...
    optional $ do
        ...
        在 POSIX sh 中，未定义 let。 warning code = parseProblem ErrorC code $ "You need a space before the " 在 POSIX sh 中，未定义 ++ 。 [c] 在 POSIX sh 中，未定义 ++ 。 "."
        case c of
            '[' -> warning 1069
            '#' -> warning 1099
            '!' -> warning 1129
    ...
    when (str /= keyword) $ do
        parseProblemAt pos ErrorC 1081 $
            "Scripts are case sensitive. Use '" 在 POSIX sh 中，未定义 ++ 。 keyword 在 POSIX sh 中，未定义 ++ 。 "', not '" 在 POSIX sh 中，未定义 ++ 。 str 在 POSIX sh 中，未定义 ++ 。 "' (or quote if literal)."
        fail ""
    return $ t id
...
readShebang = do
  ...
  where
    ...
    withHeader = try $ do
        ...
        pos <- getPosition
        anyShebang <*
            parseProblemAt pos ErrorC 1128 "shebang 必须在第一行。请删除空格并移动注释。"
...
verifyEof = eof <|> choice [
        ifParsable g_Lparen $
            parseProblem ErrorC 1088 "解析在此处终止。是否括号使用无效？",

        ifParsable readKeyword $
            parseProblem ErrorC 1089 "解析在此处终止。此关键字是否正确匹配？",

        parseProblem ErrorC 1070 "解析在此处终止。关键字不匹配或者圆括号无效？"
    ]
  where
...
readConfigFile filename = do
  ...
  where
    ...
    errorFor filename err =
        在 POSIX sh 中，未定义 let。 line = "line " 在 POSIX sh 中，未定义 ++ 。 (show . sourceLine $ errorPos err)
            suggestion = getStringFromParsec $ errorMessages err
        in
            "Failed to process " 在 POSIX sh 中，未定义 ++ 。 (e4m filename) 在 POSIX sh 中，未定义 ++ 。 ", " 在 POSIX sh 中，未定义 ++ 。 line 在 POSIX sh 中，未定义 ++ 。 ": "
                在 POSIX sh 中，未定义 ++ 。 suggestion
...
readScriptFile sourced = do
    ...
    withAnnotations rcAnnotations $ do
        ...
        在 POSIX sh 中，未定义 let。 (T_Literal _ shebangString) = shebang
        allspacing
        ...
        withAnnotations fileAnnotations $ do
            ...
            在 POSIX sh 中，未定义 let。 annotations = fileAnnotations 在 POSIX sh 中，未定义 ++ 。 rcAnnotations
            annotationId <- endSpan annotationStart
            在 POSIX sh 中，未定义 let。 shellAnnotationSpecified =
                    any (\x -> case x of ShellOverride {} -> True; _ -> False) annotations
            shellFlagSpecified <- isJust <$> Mr.asks shellTypeOverride
            在 POSIX sh 中，未定义 let。 ignoreShebang = shellAnnotationSpecified || shellFlagSpecified
            ...
            if ignoreShebang || isValidShell (executableFromShebang shebangString) /= Just False
              then do
                    ...
                    readPendingHereDocs
                    verifyEof
                    在 POSIX sh 中，未定义 let。 script = T_Annotation annotationId annotations $
  ...
  where
    ...
    isValidShell s =
        在 POSIX sh 中，未定义 let。 good = null s || any (`isPrefixOf` s) goodShells
...
-- Interactively run the comp在 POSIX sh 中，未定义 let。e parser in ghci:
...
parsesCleanly parser string = runIdentity $ do
    ...
    case (res, sys) of
        (Right userState, systemState) ->
            return $ Just . null $ parseNotes userState 在 POSIX sh 中，未定义 ++ 。 parseProblems systemState
...
getStringFromParsec errors =
        headOrDefault "" (mapMaybe f $ reverse errors)  在 POSIX sh 中，未定义 ++ 。
    ...
    where
        f err =
            case err of
                ...
                Message s     ->  if null s then Nothing else return $ s 在 POSIX sh 中，未定义 ++ 。 "."
    ...
    case result of
        Right (script, userstate) ->
            return newParseResult {
                prComments = map toPositionedComment $ nub $ parseNotes userstate 在 POSIX sh 中，未定义 ++ 。 parseProblems state,
                prTokenPositions = Map.map startEndPosToPos (positionMap userstate),
                prRoot = Just script
            }
        Left err -> do
            在 POSIX sh 中，未定义 let。 context = contextStack state
            return newParseResult {
                prComments =
                    map toPositionedComment $
                        (filter (not . isIgnored context) $
                            notesForContext context
                            在 POSIX sh 中，未定义 ++ 。 [makeErrorFor err])
                        在 POSIX sh 中，未定义 ++ 。 parseProblems state,
...
notesForContext list = zipWith ($) [first, second] [(pos, str) | ContextName pos str <- list]
  where
    first (pos, str) = ParseNote pos pos ErrorC 1073 $
        "Couldn't parse this " 在 POSIX sh 中，未定义 ++ 。 str 在 POSIX sh 中，未定义 ++ 。 ". Fix to allow more checks."
    second (pos, str) = ParseNote pos pos InfoC 1009 $
        "The mentioned syntax error was in this " 在 POSIX sh 中，未定义 ++ 。 str 在 POSIX sh 中，未定义 ++ 。 "."

-- Go over all T_UnparsedIndex and reparse them as either arithmetic or text
-- depending on 在 POSIX sh 中，未定义 declare。 -A statements.
